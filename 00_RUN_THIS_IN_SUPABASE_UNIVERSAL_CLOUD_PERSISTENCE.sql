...
-- Run once in Supabase SQL Editor before deploying v193.
...
create policy "Members read workspace documents" on public.workspace_documents for select
...
create policy "Members create workspace documents" on public.workspace_documents for insert
  to authenticated with check (public.is_workspace_member(workspace_id) and user_id = auth.uid());
drop policy if exists "Members update workspace documents" on public.workspace_documents;
create policy "Members update workspace documents" on public.workspace_documents for update
  to authenticated using (public.is_workspace_member(workspace_id))
  with check (public.is_workspace_member(workspace_id));
drop policy if exists "Managers delete workspace documents" on public.workspace_documents;
create policy "Managers delete workspace documents" on public.workspace_documents for delete
  to authenticated using (public.can_manage_workspace(workspace_id) or user_id = auth.uid());
...
create trigger workspace_documents_set_updated_at before update on public.workspace_documents
for each row execute procedure public.set_updated_at();

-- Preserve and promote existing account documents into each owner's active workspace.
...
-- Ensure the relational employee and tournament foundation exists too.
-- Safe to run after 00_ACCOUNT_DATA_FOUNDATION_RUN_IN_SUPABASE.sql.
