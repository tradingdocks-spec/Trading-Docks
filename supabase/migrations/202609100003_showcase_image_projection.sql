...
as $$
  select
    ...
    greatest(0, coalesce(nullif(i.data->>'marketPrice','')::numeric, i.inventory_value, 0)), greatest(0, i.quantity)
  from public.showcase_profiles p
  join public.workspaces w on w.id = p.workspace_id
  join public.inventory_items i on i.user_id = w.owner_id
  where p.slug = requested_slug and p.enabled and i.quantity > 0
