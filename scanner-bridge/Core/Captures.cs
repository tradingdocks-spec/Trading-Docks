...
public sealed class Captures(IWindowsScannerBackend backend) : IDisposable
{
    ...
    public void Expire()
    {
        lock (gate)
        {
            foreach (var job in jobs.Values.Where(j => j.Created < DateTimeOffset.UtcNow.AddMinutes(-2))) { job.Cancellation.Cancel(); job.Image?.Dispose(); job.Image = null; if (job.State is "ready" or "capturing") job.State = "expired"; }
    ...
    private async Task Run(Job job)
    {
        try
        {
            ...
            var image = await backend.Capture(device.Id, job.Request.Settings, new CaptureContext(job.Request.RequestId, job.Id), job.Cancellation.Token);
            ...
            lock (gate) { if (!job.Cancellation.IsCancellationRequested) { job.Image = image; job.State = "ready"; } else { image.Dispose(); job.State = "cancelled"; } }
        }
    ...
    public void Ack(string owner, string id) { lock (gate) { var job = Find(owner, id); if (job.State is not ("ready" or "acknowledged")) throw new BridgeException("CAPTURE_NOT_READY", 409); job.Image?.Dispose(); job.Image = null; job.State = "acknowledged"; } }
    public void Cancel(string owner, string id) { lock (gate) { var job = Find(owner, id); job.Cancellation.Cancel(); job.Image?.Dispose(); job.Image = null; job.State = "cancelled"; } }
