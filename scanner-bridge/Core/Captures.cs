...
public sealed class Captures(IWindowsScannerBackend backend) : IDisposable
{
    ...
    public void Expire()
    {
        lock (gate)
        {
            foreach (var job in jobs.Values.Where(j => j.Created < DateTimeOffset.UtcNow.AddMinutes(-2))) { job.Cancellation.Cancel(); job.Image = null; if (job.State is "ready" or "capturing") job.State = "expired"; }
            foreach (var key in jobs.Where(j => j.Value.Created < DateTimeOffset.UtcNow.AddMinutes(-10) && j.Value.Finished).Select(j => j.Key).ToArray()) { jobs[key].Cancellation.Dispose(); jobs.Remove(key); }
        }
    }
    ...
    private async Task Run(Job job)
    {
        try
        {
            ...
            var image = await backend.Capture(device.Id, job.Request.Settings, job.Cancellation.Token);
            if (image.Bytes.Length > Protocol.MaxImageBytes || image.Width > 3000 || image.Height > 3000 || image.Width < 1 || image.Height < 1 || image.MimeType is not ("image/jpeg" or "image/png")) throw new BridgeException("IMAGE_LIMIT_EXCEEDED");
            lock (gate) { if (!job.Cancellation.IsCancellationRequested) { job.Image = image; job.State = "ready"; } else job.State = "cancelled"; }
        }
    ...
    public void Ack(string owner, string id) { lock (gate) { var job = Find(owner, id); if (job.State is not ("ready" or "acknowledged")) throw new BridgeException("CAPTURE_NOT_READY", 409); job.Image = null; job.State = "acknowledged"; } }
    public void Cancel(string owner, string id) { lock (gate) { var job = Find(owner, id); job.Cancellation.Cancel(); job.Image = null; job.State = "cancelled"; } }
    public void RevokeAll() { lock (gate) foreach (var job in jobs.Values) { job.Cancellation.Cancel(); job.Image = null; job.State = "cancelled"; } }
