...
export function SessionProvider({ children }: PropsWithChildren) {
  ...
  useEffect(()=>{ AsyncStorage.getItem(KEY).then(raw=>{ if(raw){ try{setActive(JSON.parse(raw));}catch{} } setReady(true); }); },[]);
  const persist=async(value:WorkSession|null)=>{setActive(value); if(value) await AsyncStorage.setItem(KEY,JSON.stringify(value)); else await AsyncStorage.removeItem(KEY);};
