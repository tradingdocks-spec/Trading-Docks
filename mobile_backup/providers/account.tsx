...
export function AccountProvider({ children }: PropsWithChildren) {
  ...
  useEffect(() => { AsyncStorage.getItem(KEY).then(value => { if (value) setType(value as AccountType); setReady(true); }); }, []);
  const setAccountType = async (type: AccountType) => { setType(type); await AsyncStorage.setItem(KEY, type); };
