...
import {
  ...
  stringValue,
} from '../../src/lib/platform/billing-access-resolution.ts';
...
export async function loadMobileAccountAccessSnapshot({
  ...
  const [role, preferences, subscription, providerSubscriptions, override, workspace] = await Promise.all([
    ...
    safeList(client, 'billing_provider_subscriptions', 'provider,plan_id,status,current_period_end,updated_at', userId),
...
export function resolveMobileAccountAccessSnapshot({
  ...
  const overridePlan = stringValue(rows.override?.plan_id);
  const billingAccess = resolveBillingAccessFromRows({
    subscriptionError: null,
    subscriptionData: rows.subscription ?? null,
    providerRows: rows.providerSubscriptions ?? null,
    overridePlan,
    now,
  });
