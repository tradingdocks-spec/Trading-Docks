...
export const DEFAULT_LIVE_FRAME_ANALYSIS_CONFIG: Omit<LiveFrameAnalysisConfig, 'guide'> = {
  thresholds: { ...DEFAULT_CONTINUOUS_SCANNER_THRESHOLDS, requiredStabilityMs: 650 },
...
export function analyzeLiveFrame(
  ...
  const cornersVisible = Boolean(bounds && aspectRatioOk);
  ...
  const perspectiveScore = bounds ? Math.abs(aspectRatio - TRADING_CARD_GUIDE_RATIO) / TRADING_CARD_GUIDE_RATIO : 1;
  ...
  return {
    ...
    aspectRatio,
    aspectRatioOk,
    crop: readyForAutoCapture ? crop : null,
  };
}
...
function motionBetweenFrames(previous: LiveFrameSample, current: LiveFrameSample) {
  ...
  return Math.max(0, Math.min(1, (count ? delta / count : 255) / 255));
}
