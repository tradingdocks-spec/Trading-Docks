...
import { DockHeader, DockSurface, DockTray, TDBadge, TDButton, TDListRow, TDNavigationHeader, TDSectionHeader, TDStatusIndicator, TDText } from '@/components/design-system';
...
export default function Profile() {
  ...
  return (
    <ScrollView style={s.page} contentContainerStyle={[s.content, { paddingTop: Math.max(insets.top + 26, 56), paddingBottom: getMobileScrollBottomInset(insets.bottom) }]} showsVerticalScrollIndicator={false}>
      ...
      <DockTray style={s.aboutCard}>
        <DockHeader title={version.name} subtitle={`Version ${version.version}`} right={<TDText variant="caption" tone="muted">Build {version.build}</TDText>} />
      </DockTray>
