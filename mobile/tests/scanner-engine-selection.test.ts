...
test('scanner engine defaults to prebuilt and keeps legacy as dev-only fallback', () => {
  ...
  assert.equal(resolveScannerEngineMode({ NODE_ENV: 'development', EXPO_PUBLIC_SCANNER_ENGINE: 'legacy' }), 'legacy');
...
test('normal scanner routes resolve through the engine selector instead of the legacy screen directly', () => {
  ...
  assert.match(automatic, /scanner-engine-screen/);
  assert.match(single, /scanner-engine-screen/);
  assert.match(nativeSelector, /PrebuiltScannerScreen/);
  assert.match(nativeSelector, /AutomaticScannerScreen/);
