...
const tabs: { id: AdminTab; label: string; icon: typeof Activity }[] = [
  ...
  { id: "preview", label: "Plan Preview", icon: Eye },
    ...
    <div className="min-h-[calc(100vh-72px)] bg-[#02090f]">
      ...
      <div className="mx-auto grid max-w-[1500px] gap-6 px-4 py-6 sm:px-8 lg:grid-cols-[230px_minmax(0,1fr)]">
        ...
        <main className="min-w-0">
          ...
          {tab === "preview" ? <PlanPreviewPanel /> : null}
...
function PlanPreviewPanel() {
  const plans = [
    { id: "free", label: "Free", detail: "No Inventory access; shows upgrade prompts.", tone: "border-slate-300/15 bg-slate-300/[0.035] text-slate-200" },
    { id: "collector", label: "Collector", detail: "Personal collection, storage, search, and put-away tools.", tone: "border-cyan-300/15 bg-cyan-300/[0.035] text-cyan-100" },
    { id: "seller", label: "Seller", detail: "Marketplace listings, pricing queues, and seller operations.", tone: "border-violet-300/15 bg-violet-300/[0.035] text-violet-100" },
    { id: "business", label: "Store", detail: "Complete command center, analytics, profitability, and team tools.", tone: "border-amber-300/15 bg-amber-300/[0.035] text-amber-100" },
  ] as const;

  return (
    <section className="rounded-[24px] border border-white/[0.07] bg-[#06121b] p-5 sm:p-6">
      <div className="flex items-start gap-4">
        <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-cyan-300/15 bg-cyan-300/[0.055] text-cyan-200"><Eye className="h-5 w-5" /></span>
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-cyan-300/65">Owner testing</p>
          <h2 className="mt-1 text-xl font-semibold text-white">View website as a plan</h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Temporarily test the dashboard with a customer plan. Your permanent Owner account and real subscription are never changed.</p>
        </div>
      </div>
      <div className="mt-7 grid gap-3 sm:grid-cols-2">
        {plans.map((plan) => (
          <a key={plan.id} href={`/dashboard/admin/preview?plan=${plan.id}&next=/dashboard`} className={`group rounded-2xl border p-5 transition hover:-translate-y-0.5 hover:brightness-125 ${plan.tone}`}>
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm font-semibold">View as {plan.label}</p>
              <ChevronRight className="h-4 w-4 opacity-55 transition group-hover:translate-x-0.5" />
            </div>
            <p className="mt-2 text-[11px] leading-5 text-slate-500">{plan.detail}</p>
          </a>
        ))}
      </div>
      <div className="mt-5 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-emerald-300/10 bg-emerald-300/[0.025] p-4">
        <div><p className="text-xs font-semibold text-emerald-100">Finished testing?</p><p className="mt-1 text-[10px] text-emerald-100/45">Clear the preview and restore full Owner access.</p></div>
        <a href="/dashboard/admin/preview?next=/dashboard/admin" className="rounded-xl border border-emerald-300/15 bg-emerald-300/[0.07] px-4 py-2.5 text-xs font-semibold text-emerald-100 transition hover:bg-emerald-300/[0.12]">Return to Owner View</a>
      </div>
    </section>
  );
}
