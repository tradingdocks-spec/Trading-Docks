...
test("SSR and reduced-motion snapshots use identical stable quotes without hydration warnings", async ({ browser, page }) => {
  const serverContext = await browser.newContext({ javaScriptEnabled: false });
  const serverPage = await serverContext.newPage();
  await serverPage.goto("http://localhost:4186/#market");
...
test("one quote ticks at a time, layout stays stable, pause and offscreen stop updates", async ({ page }) => {
  ...
  const bounds = await page.locator("[data-artwork-mode]").boundingBox();
  ...
  expect((await page.locator("[data-artwork-mode]").boundingBox())?.height).toBe(bounds?.height);
