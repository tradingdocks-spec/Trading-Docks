...
test("selected-product layout uses readable SKU controls instead of raw SKU IDs", () => {
  ...
  assert.match(page, /Spread/);
  assert.match(page, /BuyingSummaryBlock/);
  assert.match(page, /BuyingMetric/);
  ...
  assert.match(page, /\+\$\{props\.buyingRule\.storeCreditBonusPercent\}% bonus/);
  ...
  assert.match(page, /Edit rule|Set buying rule/);
  assert.match(page, /href="\/dashboard\/buying-rules"/);
  assert.doesNotMatch(page, /Rule source/);
