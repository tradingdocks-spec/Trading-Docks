...
test("signed bridge protocol: pairing, profiles, test scan, 100 cards, review, real commit, next batch", async ({ page }) => {
  ...
  await page.getByRole("combobox", { name: "Scanner profile" }).selectOption("quality");
  ...
  await page.getByRole("button", { name: "Start Continuous Scan", exact: true }).click();
  await expect(page.getByRole("progressbar")).toHaveAttribute("aria-valuenow", "100", { timeout: 120_000 });
  await expect(page.getByRole("button", { name: "Start Continuous Scan", exact: true })).toBeDisabled(); await expect(page.getByRole("button", { name: "Scan One", exact: true })).toBeDisabled();
...
test("absent and incompatible bridge preserve Upload and CSV", async ({ page }) => {
  const state = await mock(page); state.offline = true; await open(page); await expect(page.getByText(/Needs setup. Start the installed Scanner Agent/)).toBeVisible();
...
test("external-settings backend waits for physical output, previews, cancels and resumes without vendor branches", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Scanner Settings", exact: true }).click();
  await expect(page.getByText("Use the configured Trading Docks card profile.", { exact: false })).toBeVisible();
