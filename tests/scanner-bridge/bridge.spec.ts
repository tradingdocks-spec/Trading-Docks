...
test("remembered scanner, missing device, jam, cancellation and disconnect retain draft", async ({ page }) => {
  ...
  state.hold = true; await page.getByRole("button", { name: "Start Continuous Scan", exact: true }).click(); await page.getByRole("button", { name: "Cancel Current Scan", exact: true }).click(); await expect(page.getByRole("button", { name: "Scan One", exact: true })).toBeEnabled(); expect(state.cancelled).toBeGreaterThan(0); state.hold = false;
