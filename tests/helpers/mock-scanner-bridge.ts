...
export async function mock(page: Page) {
  ...
  const state = { captures: 0, captureRequests: 0, ack: 0, interrupted: false, durable: false, version: 1, offline: false, missing: false, fail: "", cancelled: 0, hold: false, external: false };
  ...
  await page.route("https://127.0.0.1:47391/**", async route => {
    ...
    if (path === "/v1/health") return reply({ running: true, bridgeVersion: "1.2.0", protocolVersion: state.version, automaticInbox: true, durableRecovery: state.durable });
    ...
    if (path === "/v2/pair/renew") return reply({ expires: new Date(Date.now() + 86400000).toISOString() });
    ...
    if (path === "/v1/capture") { state.captureRequests++; if (!ids.has(data.requestId)) { ids.set(data.requestId, crypto.randomUUID()); state.captures++; } return reply({ captureId: ids.get(data.requestId) }); }
    ...
    if (path.startsWith("/v1/capture/")) return reply(cancelledIds.has(path.split("/")[3]) ? { status: "cancelled", error: "CAPTURE_CANCELLED" } : state.interrupted ? { status: "interrupted", error: "CAPTURE_INTERRUPTED" } : state.fail ? { status: "failed", error: state.fail } : state.hold ? { status: "capturing" } : { status: "ready", image: validImage, mimeType: "image/png", width: 1, height: 1 });
