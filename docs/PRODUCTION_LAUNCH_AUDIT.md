...
| P0 | Deck Architect repaired engine must be certified in the actual authenticated rendered product before it can move from fail to pass. | Blocked | Engine/source tests pass for Pauper and Krenko on this branch, but authenticated production browser QA still rendered the old Pauper draft-shell failure with `Not calculated` metrics and illegal Pauper intelligence. Deploy the repaired branch to a preview/release candidate, then rerun the checklist in `docs/PRODUCTION_LAUNCH_CERTIFICATION.md`. |
...
- Blocked: Deck Architect rendered-result certification failed against the currently deployed production surface. Automated engine checks prove the repaired paths in this branch, but the deployed product cannot be marked PASS until the repaired branch is deployed and the Pauper/Krenko browser scenarios in `docs/PRODUCTION_LAUNCH_CERTIFICATION.md` succeed.
...
| Deck Architect | Blocked | Current production browser QA failed the Pauper certification gate. Deploy this branch to a preview/release candidate, then rerun authenticated certification for Pauper / Collection Optimized / Competitive and Commander / Krenko, Mob Boss; engine tests alone are insufficient for PASS. |
