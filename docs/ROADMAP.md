...
- Implemented: Redesign scanner navigation into Scanner Product V3 with a Scan Modes tab route, dedicated Automatic Scan route, dedicated Single Scan route, Review List post-processing, simplified settings hierarchy, and no fake Grid Scan route.
- Implemented: Add a shared scanner readiness language for Single Scan and Automatic Scan with cyan Searching, amber Needs Attention, emerald Ready, blue Processing, one user-facing instruction, advisory Single Scan quality, and readiness-gated automatic capture.
...
- Partially Implemented: Rapid Scan is now the active Automatic Scan presentation and has iOS native frame-title OCR wired into bundled local matching/session append, but true arbitrary-card throughput remains blocked by physical benchmark validation and exact-printing live refinement.
...
- Implemented: Wire Rapid Scan and Single Scan into the shared multi-signal recognition service, including active diagnostics and offline visual-only recovery tests.
- Partially Implemented: Visual recognition is currently a compact perceptual-fingerprint prototype backed by a 9-record descriptor seed, not a benchmarked production artwork recognizer. Production work still needs a full Scryfall artwork-derived descriptor index, native perspective rectification, and physical benchmark results.
...
- Partially Implemented: iOS captured-still OCR and shared local Magic title identity are implemented, but artwork matching, set-symbol recognition, perspective correction, Android OCR, and foil classification remain provider contracts until benchmarked; the scanner does not fake recognition accuracy.
