...
- Planned: Promote location-first Collection behavior as a core capability: every owned-card search result should show physical lot/location breakdowns, CSV import should support importing directly into a selected location, and Card Workspace/Collection should support partial-quantity moves without flattening all copies into one pool. See `docs/COLLECTION_LOCATION_AUTHORITY.md`.
