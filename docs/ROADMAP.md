...
- Partially Implemented: Rapid Scan is now the active Automatic Scan presentation and has iOS native frame-title OCR wired into local matching/session append, but true arbitrary-card throughput remains blocked by full local MTG catalog packaging and physical benchmark validation.
