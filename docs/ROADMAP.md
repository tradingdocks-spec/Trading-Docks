...
- Implemented: Calibrate Magic still-capture OCR for physical iPhone retry flow by normalizing rotated still dimensions, adding primary/expanded/lower/full-card title fallback regions, exposing development-only crop proof, and ensuring Retake resumes the camera without entering the user-paused state.
