...
## Phase 6 Terminal extension

Implemented: the existing `SQUARE` adapter routes `square_terminal` attempts through Square Terminal checkout. Server-resolved register assignments and frozen device/location context extend the immutable attempt; Terminal checkout IDs and Payment IDs are separate. Signed Terminal/device events use the existing webhook inbox and authoritative retrieval. The original-cashier sale/refund finalizers remain the only inventory boundary.

Pairing, hardware administration, scope reauthorization, cancellation and recovery are described in [POS_SQUARE_TERMINAL.md](POS_SQUARE_TERMINAL.md). Ordinary US refunds retain the Refunds API; only Square's nested card-presence flag selects the provider-specific Terminal refund contract. Local deterministic acceptance is implemented; actual Sandbox configuration and physical hardware acceptance remain outstanding. Production remains blocked.
