...
- Partially Implemented: Physical-device testing still shows Rapid Scan and Single Scan can fail on readable Magic cards such as Goblin War Strike, confirming OCR-only identity is not robust enough. Rapid and Single now call the shared multi-signal recognition service, but production-scale visual reference indexing and physical benchmark evidence remain unfinished.
...
- Implemented: Focused multi-signal recognition tests cover default descriptor-index loading, visual-fingerprint plus weak OCR identity, OCR-only review behavior, visual/OCR conflict review behavior, unusable-frame rejection, best-frame selection, printing refinement after identity, Rapid active fusion recovery, Single active fusion recovery, and benchmark comparison without fake compact-embedding measurements.
