...
- Implemented: The active Scan tab now uses a premium camera-first hierarchy with compact two-line header, large guide viewport, exactly three primary controls, secondary settings/manual/diagnostics panels, and a compact Review List chip.
...
- Partially Implemented: Batch scanner latency diagnostics are available in the development panel, but capture/OCR/Scryfall/session-write budgets still need measured physical-device benchmark runs.
