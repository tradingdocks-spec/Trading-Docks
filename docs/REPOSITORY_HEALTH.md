...
- Partially Implemented: Physical-device testing still shows Rapid Scan and Single Scan can fail on readable Magic cards such as Goblin War Strike, confirming OCR-only identity is not robust enough. A shared multi-signal recognition decision model now exists, but production visual reference indexing and scanner UI integration remain unfinished.
...
- Implemented: Focused multi-signal recognition tests cover visual-fingerprint plus weak OCR identity, OCR-only review behavior, visual/OCR conflict review behavior, unusable-frame rejection, best-frame selection, printing refinement after identity, and benchmark comparison without fake compact-embedding measurements.
