...
- Implemented: The active Scan tab now uses a premium camera-first hierarchy with compact HUD, large guide viewport, result tray, secondary settings/manual/diagnostics panels, and bottom session bar.
- Implemented: Magic still-capture OCR now normalizes rotated iPhone captures to the live preview orientation, uses primary/expanded/lower/full-card title OCR fallback regions, exposes development-only crop proof, and keeps failed Retake recovery out of the user-paused camera state.
...
- Implemented: Focused premium scanner experience tests cover diagnostics separation, failed OCR recovery without unknown-session insertion, high-confidence and ambiguous result trays, pipeline state mapping, text-plus-color guide states, and high-volume defaults without auto inventory insertion.
