...
- Partially Implemented: Batch scanner latency diagnostics are available in the development panel with bounded local history, measured averages, preview/capture resolution when available, and sanitized JSON export; capture/OCR/Scryfall/session-write budgets and camera FPS still need measured physical-device benchmark runs.
