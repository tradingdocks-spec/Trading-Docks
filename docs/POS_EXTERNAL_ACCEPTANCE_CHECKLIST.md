...
2. Supply an isolated HTTPS staging/test origin, represented here by `<STAGING_HTTPS_ORIGIN>`. No public origin was provisioned in Phase 7B. This implementation intentionally blocks Square starts in a production-mode Next build: use the isolated development/test runtime for Sandbox validation. Production Square support needs a separate reviewed change.
