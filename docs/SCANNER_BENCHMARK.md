...
- Implemented: The development Recognition Lab can emit multi-signal diagnostics for geometry, descriptor generation, visual lookup, OCR, fusion, identity, and printing-refinement timing buckets. The production scanner currently uses captured-still OCR plus local Magic-name identity; documentation must not substitute estimates for physical measurements.
- Implemented: The bundled visual descriptor index now contains 57,519 production-scale Scryfall reference-image descriptors with versioned metadata. It replaces the previous 9-record seed and remains descriptor-only; source card images are not included in Git or the app bundle.
- Implemented: Local Node measurement on August 9, 2026 loaded the generated descriptor module in 1,876.878 ms, built/prewarmed the bucketed matcher in 47.041 ms, used an estimated 74,813,592 heap bytes, matched a broad Goblin War Strike bucket in 4.8263 ms across 2,674 candidates, and matched the same descriptor with OCR oracle narrowing in 0.0428 ms across 1 candidate. These are development-machine measurements, not physical-device scanner latency claims.
- Implemented: The development-only Scanner Recognition Lab runs a bake-off on one captured physical card image through OCR Accurate, current `luma_phash_8x8_v1`, and an Apple Vision Feature Print prototype. It reports identity outcome, top-1/top-5 candidates, latency, input quality, and opt-in debug artifacts without changing production scanner behavior.
- Requires Production Configuration: Apple Vision Feature Print testing requires a fresh native iOS development/preview build because `mobile/modules/trading-docks-vision-ocr` now exposes `VNGenerateImageFeaturePrintRequest` diagnostic APIs.
- Partially Implemented: The active descriptor method remains `luma_phash_8x8_v1`. It is simple and local-first, but physical benchmark evidence is still required before changing thresholds or claiming recognition accuracy under perspective, glare, sleeves, low light, rotation, or partial crops.
