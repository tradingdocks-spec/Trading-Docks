...
- Implemented: Automatic Scan now defaults to the Rapid Scan presentation, suppresses still-photo auto-capture in Rapid mode, samples live frames through native title OCR, and appends local high/medium title matches into the review session using the bundled local Magic name catalog.
