...
- Implemented: The production scanner uses Vision/frame analysis for guidance and auto-capture gates, then invokes the same still-capture OCR pipeline for Auto Scan ON and OFF. It does not use live title OCR, pHash, visual descriptors, Feature Print, or fusion as production identity authority.
...
- Partially Implemented: Camera capture is local-first and includes iOS captured-still OCR, but it still does not include benchmarked artwork recognition, set-symbol recognition, perspective correction, Android OCR, or finish classification.
- Partially Implemented: Auto Scan ON is wired through the same still-capture path as manual Capture, but physical-device Golden 50 testing must prove trigger/rearm reliability before cards-per-minute optimization.
