...
- Implemented: The active Scan screen now uses the premium camera-first hierarchy documented in `docs/SCANNER_PRODUCT_EXPERIENCE.md`: compact two-line header, large camera viewport, Torch/Capture/Search controls, compact result tray, compact session strip, and secondary panels for settings, manual search, and diagnostics.
