...
- Partially Implemented: Automatic Scan now defaults to the Rapid Scan presentation, suppresses still-photo auto-capture in Rapid mode, samples live frames through native title OCR, and appends local high/medium title matches into the review session. Full arbitrary-card throughput still requires a bundled local MTG name catalog and physical benchmarks.
...
- Partially Implemented: Rapid Scan has an iOS native frame-to-title-ROI OCR provider. Android live OCR, full local catalog coverage, and benchmarked throughput remain planned.
