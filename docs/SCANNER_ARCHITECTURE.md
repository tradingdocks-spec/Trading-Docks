...
- Partially Implemented: `mobile/services/generated/magic-visual-descriptor-index.ts` provides a compact local visual descriptor seed covering the physical regression set without bundling card images. It proves offline index loading and scanner wiring, but it is not yet the full artwork-derived Scryfall descriptor corpus.
...
- Partially Implemented: `ArtworkMatchingProvider` contracts support layout and artwork fingerprint observations. A compact luma perceptual-fingerprint prototype is now active in Rapid and Single recognition paths, but a production full Scryfall reference-image index and benchmarked artwork-similarity engine are not active yet.
...
- Implemented: Refresh command contract: run `npm run catalog:magic-visual-descriptors -- --source ./tmp/scryfall-visual-descriptors.json --out services/generated/magic-visual-descriptor-index.ts` from `mobile/` after preparing compact Scryfall artwork descriptors locally.
- Planned: Replace the current descriptor seed with a production Scryfall reference descriptor index from canonical card images, including record count, storage size, refresh cadence, and lookup-complexity validation.
