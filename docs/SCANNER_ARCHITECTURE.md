...
- Implemented: `mobile/services/magic-card-identity.ts` provides the shared local Magic title identity layer for the unified scanner.
...
- Implemented: `scanner-multi-signal-recognition.ts`, visual descriptors, pHash-style matching, and Apple Vision Feature Print comparison are retained for the development-only Scanner Recognition Lab, not the production scanner.
- Implemented: `mobile/services/generated/magic-visual-descriptor-index.ts` is generated from Scryfall `default_cards` reference images with compact 8x8 luma descriptors only. The current generated baseline covers 37,553 oracle identities, 57,519 unique artwork candidates, 111,786 paper printing references, and 57,519 shipped descriptor records with zero image failures in the final generation run. The generated TypeScript file is 8,895,151 bytes on disk and does not bundle source card images.
- Implemented: `mobile/services/scanner-recognition-lab.ts` is a development-only bake-off harness for one captured physical card image. It compares OCR Accurate, current pHash, and Apple Vision Feature Print candidates against the same image input and records input quality before any production scanner change is made.
...
- Implemented: Scanner Product V3 consolidates production scanning into one unified scanner with Auto Scan ON/OFF capture behavior. See `docs/SCANNER_PRODUCT_V3.md`.
- Implemented: `/scan/automatic` is the production scanner route. `/scan/single` remains a compatibility alias to the same scanner and does not define a second production recognition engine.
...
- Implemented: `mobile/services/rapid-scan-pipeline.ts` defines Rapid Scan and Precision Scan mode contracts, fixed scan-zone ROIs, card-change state, shared local Magic name matching, confidence routing, result-tray helpers, batch destination inheritance, and benchmark metrics. See `docs/RAPID_SCAN_PIPELINE.md`.
- Implemented: `TradingDocksVisionOcr.recognizeFrameTitle` adds an iOS Apple Vision live-frame title OCR path for bounded luma frames and normalized title ROI without base64 or temporary image files.
- Implemented: `TradingDocksVisionOcr.detectCardRectangle` adds iOS Apple Vision `VNDetectRectanglesRequest` support for captured stills, returning detected status, confidence, normalized corners, normalized bounding box, aspect ratio, and duration without uploading or retaining source images.
- Implemented: The production scanner uses Vision/frame analysis plus the `apple-vision-auto-capture` state machine for guidance and Auto Scan ON triggering, then invokes the same still-capture OCR pipeline for Auto Scan ON and OFF. It does not use live title OCR, pHash, visual descriptors, Feature Print, or fusion as production identity authority.
...
- Partially Implemented: Camera capture is local-first and includes iOS captured-still OCR plus captured-still Apple Vision rectangle detection, but it still does not include benchmarked artwork recognition, set-symbol recognition, true perspective-warp bitmap output, Android OCR, or finish classification.
- Partially Implemented: Auto Scan ON is wired through the same still-capture path as manual Capture and now has an explicit SEARCHING/FOUND/STABILIZING/READY/CAPTURING/READING/IDENTIFIED/WAITING_FOR_REMOVAL/REARMED state model, but physical-device Golden 50 testing must prove trigger/rearm reliability before cards-per-minute optimization.
- Planned: `docs/SCANNER_AUTO_CAPTURE_NATIVE_PLAN.md` remains the native frame-delivery and QA reference for future hands-free improvements.
...
- Partially Implemented: Rapid/live OCR services and multi-signal fusion remain source-tested experimental services, but they are not active in the normal production scanner path.
- Partially Implemented: `ArtworkMatchingProvider` contracts support layout and artwork fingerprint observations. A production-scale Scryfall reference-image descriptor index is available for lab comparison, but the luma perceptual-fingerprint method still needs physical benchmark evidence before production use, accuracy claims, or threshold changes.
...
- Implemented: Diagnostics report camera readiness, scanner lifecycle state, capture ID, preview dimensions, 63:88 guide dimensions, normalized guide crop, capture state, duplicate/removal state, recognition stage, session insertion result, timing buckets, captured-still orientation normalization, OCR crop pixel rectangles, selected/rejected title attempts, Scryfall outcome, local crop-proof overlays, Vision ROI, OCR/match durations, and catalog readiness.
...
- Implemented: The lab multi-signal model treats OCR as one signal rather than the only identity source: strong visual plus weak OCR can identify the card, strong OCR without visual evidence routes to review, visual/OCR conflict routes to review, and unusable geometry rejects the frame before identity fusion.
- Partially Implemented: The active mobile UI uses captured-still OCR, local Magic identity, manual Scryfall search, and the Magic recognition adapter to show recognized/likely/ambiguous/manual-review states and top alternatives. Visual image recognition remains lab-only until benchmarked.
...
- Implemented: Refresh command contract: run `npm run catalog:magic-visual-index` from `mobile/`. The command fetches Scryfall `default_cards`, downloads paper reference images into ignored `mobile/.cache/scryfall-card-images/`, normalizes each image to the same luma8 8x8 mean-hash contract used by runtime frame crops, writes compact descriptors plus version metadata, validates the regression-card list, and leaves source images out of the runtime bundle.
- Implemented: The descriptor index uses identity-first lookup with hash buckets and OCR-assisted oracle narrowing where available. The current descriptor schema is version 2, descriptor version `luma_phash_8x8_v1`, normalization version `scryfall_full_card_luma8_8x8_mean_v1`.
- Planned: Future catalog updates should keep a bundled baseline but add a downloadable incremental descriptor index so new Magic sets do not require an App Store release.
