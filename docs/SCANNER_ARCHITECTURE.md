...
- Implemented: Rapid Scan and Single Scan now pass active Vision Engine evidence into `scanner-multi-signal-recognition.ts`; diagnostics expose geometry quality, best-frame score, visual candidate, visual similarity, OCR candidate, fused candidate, fused confidence, printing candidate, final decision, and timing buckets.
...
- Implemented: Automatic Scan now defaults to the Rapid Scan presentation, suppresses still-photo auto-capture in Rapid mode, samples live frames through the Vision Engine and native title OCR, fuses visual fingerprint plus OCR evidence, and appends local high-confidence identities into the review session.
...
- Partially Implemented: Rapid Scan has an iOS native frame-to-title-ROI OCR provider, bundled Magic name identity, and active multi-signal fusion. Android live OCR, exact-printing live refinement, and benchmarked throughput remain planned.
- Partially Implemented: `ArtworkMatchingProvider` contracts support layout and artwork fingerprint observations. A production-scale Scryfall reference-image descriptor index is active in Rapid and Single recognition paths, but the luma perceptual-fingerprint method still needs physical benchmark evidence before accuracy claims or threshold changes.
...
- Implemented: Diagnostics report camera readiness, scanner lifecycle state, capture ID, preview dimensions, 63:88 guide dimensions, normalized guide crop, capture state, duplicate/removal state, recognition stage, session insertion result, visual index count, visual candidate, visual similarity, fused candidate, fused confidence, final decision, timing buckets, captured-still orientation normalization, OCR crop pixel rectangles, selected/rejected title attempts, Scryfall outcome, local crop-proof overlays, Rapid raw/normalized OCR, Rapid match score/band, Rapid ROI, Vision ROI, OCR/match durations, and catalog readiness.
...
- Partially Implemented: The active mobile UI uses manual Scryfall search, the Magic recognition adapter, and the multi-signal fusion service to show recognized/likely/ambiguous/manual-review states, top alternatives, per-signal confidence, and "Why this match?" details. Visual image recognition is active as an offline compact-descriptor signal with production-scale Scryfall coverage, but it is not presented as benchmarked production artwork accuracy.
