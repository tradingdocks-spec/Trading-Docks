...
| AUD-004 | Inventory, Chaos Sort, imports, marketplace imports, orders | Data integrity | P1 | Verify duplicate submission, retry, zero quantity, location/provenance preservation, idempotency, and cross-user mutation behavior with live rows. | Deployed migration state still requires staging replay even though the supplied production schema review confirmed the relevant authority objects. | Run staging transaction/concurrency checks and verify migration history before release. | **Code verified; staging replay pending** | inventory events, collector API, Chaos Sort, identity, security tests |
...
- Staging verification is still required for `SECURITY DEFINER` execution grants, search paths, views, storage policies, and migration drift before production application; the supplied live review confirms the current production authority state but was not replayed by this shell session.
...
The current canonical role model is `public.user_roles(user_id, role)` with `admin_role` values including `owner`, and `current_admin_role()` / `is_admin()` already read that table. The supplied production verification confirms the existing Trading Docks owner identity already has `user_roles.role = 'owner'`, so the one-time bootstrap is idempotent against the current authority state. The legacy email lookup remains only in that bootstrap block; it is not used by any runtime function. This avoids treating mutable email identity as permanent authority and separates platform authority from billing/membership labels.

The proposed `20260917154750_platform_owner_role_authority_transition.sql` preserves bootstrap access transactionally by requiring the existing bootstrap identity to exist, inserting/updating its `user_roles` row to `owner`, then switching `is_platform_owner()`, `protect_platform_owner()`, `admin_set_membership_override()`, and `admin_directory()` to trusted role authority. It also blocks membership overrides for any role-table owner rather than one email. For admin-directory display only, a role-table owner maps to the canonical `store` full-access equivalent rather than `free`; this does not turn platform authority into a commercial subscription. This migration is intentionally **not applied**. If the bootstrap identity is absent, it fails rather than silently locking out the owner.

The transition protects the role through the existing trigger path: an owner row cannot be deleted or downgraded through normal mutation. Its four redefined `SECURITY DEFINER` functions use `set search_path = pg_catalog` with schema-qualified project objects. The final staging order remains search-path hardening, privilege hardening, then owner-role transition. PostgreSQL `CREATE OR REPLACE FUNCTION` preserves existing function privileges, and the transition migration contains no grant/revoke statements that would broaden that earlier boundary.
...
3. Stage and verify `20260917154750_platform_owner_role_authority_transition.sql` only after reviewing the bootstrap guard and owner-display mapping.
4. Only after advisor re-check and query-plan review, propose separate high-value index/RLS optimization migrations.
5. Handle leaked-password protection as an owner-controlled Auth configuration change, not SQL.
