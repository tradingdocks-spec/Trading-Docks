...
- Implemented: Commander strategy intelligence uses a provider/domain layer rather than UI hard-coding. Structured Trading Docks-authored strategy profiles currently cover known commander examples such as Nekusar, Muldrotha, and Atraxa, while a deterministic rules-text fallback remains available for other commanders.
...
- Implemented: The role classifier now centralizes ramp, mana rocks, mana dorks, rituals, Treasure generation, cost reduction, color fixing, land fixing, card draw, card advantage, wheels, group draw, draw punishment, conditional draw, incidental draw, cantrips, hand cycling, targeted and mass removal, countermagic, protection, tutor, recursion, graveyard interaction, token generation, sacrifice, discard, threats, finishers, synergy, combo pieces, lifegain, burn, and artifact/enchantment interaction signals. Mono-color Commander builds do not create an artificial color-fixing quota.
