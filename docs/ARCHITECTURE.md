...
- Implemented: Card Workspace uses the shared cost-basis model for `Your Position`, shows known/partial/missing cost coverage, and shows unrealized gain/loss only when both saved valuation and known cost basis exist.
- Partially Implemented: `supabase/migrations/202608120002_inventory_financial_events_proposal.sql` proposes an additive, append-only `inventory_events` ledger with authenticated user scoping, optional workspace/item/location references, event type constraints, idempotency, timestamps, and RLS. The migration is not applied automatically by application code.
- Partially Implemented: The Collector Workspace mutation route emits best-effort ledger events for quantity, condition, finish, and storage changes. The helper is safe before the ledger migration exists and logs non-secret server warnings for unexpected event-write failures. Because current route handlers are not transaction-wrapped, future work should move inventory mutation plus event insertion into a database RPC for strict atomicity.
- Planned: Marketplace sale, return, import, scanner, CSV, deletion/archive, price-update, and cost-basis-update event emission should be added at each mutation authority after those paths are reviewed. No fake historical FIFO or inferred sale-to-lot allocation is active.
- Deferred: Historical backfill is intentionally not performed. If operators approve a baseline, records must use the `baseline_snapshot` event type and must not be presented as observed historical movement.
- Security: Financial data, cost basis, profit, inventory history, and workspace linkage are private account data. Active loaders scope reads by authenticated user id; server routes must not trust client-supplied user/workspace identifiers for financial authority.
- Performance: Current Card Workspace history reads are bounded to 12 recent events. Inventory Attention uses database-level counts plus bounded representative samples. Seller summaries load bounded order/inventory/listing windows and remain compatible with future materialized financial summaries.
