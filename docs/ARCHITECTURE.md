...
- Implemented: The role classifier now centralizes ramp, mana rocks, mana dorks, rituals, Treasure generation, cost reduction, color fixing, land fixing, card draw, card advantage, targeted and mass removal, countermagic, protection, tutor, recursion, graveyard interaction, token generation, sacrifice, discard, threats, finishers, synergy, combo pieces, lifegain, burn, and artifact/enchantment interaction signals. Mono-color Commander builds do not create an artificial color-fixing quota.
...
- Implemented: `src/lib/deck-architect/commander-meta-provider.ts` defines an EDHREC-compatible provider boundary, but EDHREC is not integrated, scraped, crawled, mirrored, or used as a backend. A licensed implementation can be attached later after commercial permission is secured.
