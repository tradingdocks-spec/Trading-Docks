...
- Partially Implemented: The workspace evaluates collection-derived requirements, owned commander candidates, Pauper archetype opportunities, Commander strategy signals, and review-first add/substitution recommendations. It does not yet persist deck proposals or apply mutations to Deck Vault.
...
- Implemented: The customer-facing workspace now uses progressive build flow states, hides Buildability and Deck Health scores until a complete working deck target exists, and keeps save/apply actions unavailable until Deck Vault proposal persistence is implemented.
...
- Planned: Add external provider-backed archetype/template sources, saved proposal persistence, apply/revert workflows against Deck Vault, upgrade paths from existing Deck Vault decks, combo provider activation, and richer price/provider coverage after the deterministic recommendation layer is reviewed.
