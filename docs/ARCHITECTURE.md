...
- Implemented: The production Deck Architect UI no longer uses the legacy owned-card pile helper for non-Commander builds. Pauper and future full-intelligence constructed formats run through `constructValidatedFormatDeck`, which returns a normalized result containing status, strategy, requirements, validation, ownership, buildability, health, missing cards, machine-readable recommendation reasons, and diagnostics.
...
- Implemented: Deck Vault handoff is fail-closed. Deck Architect can display setup, intelligence, and failure states, but only `complete` validated builds can be saved/opened in Deck Builder. `draft_shell` and `failed` outputs remain review data, not finished decklists.
...
- Implemented: Constructed-format candidate selection rejects known illegal collection cards for the selected format and preserves exact owned/missing quantities in ownership overlays. Pauper regression coverage explicitly prevents Commander-only staples from entering a Pauper output.
