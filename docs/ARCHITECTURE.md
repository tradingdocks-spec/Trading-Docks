...
- Implemented: Commander strategy intelligence uses a provider/domain layer rather than UI hard-coding. Structured Trading Docks-authored strategy profiles currently cover known commander examples such as Muldrotha and Atraxa, while a deterministic rules-text fallback remains available for other commanders.
...
- Implemented: The role classifier now centralizes ramp, mana fixing, card draw, card advantage, targeted and mass removal, countermagic, protection, tutor, recursion, graveyard interaction, token generation, sacrifice, discard, threats, finishers, synergy, combo pieces, lifegain, burn, and artifact/enchantment interaction signals.
...
- Provider research: Current production intelligence uses Trading Docks-authored domain data plus existing Scryfall legality metadata saved with collection/catalog records when available. TCGplayer/TCGTracking pricing and catalog identifiers remain price/identity providers, not decklist authorities. Commander Spellbook remains a future combo provider boundary; do not copy or scrape combo content outside an approved API/licensing path.
- Planned: Add external provider-backed archetype/template sources with terms review, saved proposal persistence, Deck Vault proposal history, combo provider activation, and richer price/provider coverage after the deterministic recommendation layer is reviewed.
