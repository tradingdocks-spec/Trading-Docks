...
- Implemented: Scanner Product V3 now presents one Trading Docks Scanner entry and Review List access. The scanner uses one instruction, a compact Auto Scan ON/OFF header control, Torch, Capture when Auto Scan is OFF, and a compact Review affordance without exposing separate production scanner modes.
