...
export default async function ChaosSortBatchDetailPage({ params }: { params: Promise<{ batchId: string }> }) {
  ...
  const [{ data: positions }, { data: session }] = await Promise.all([
    supabase.from("chaos_sort_inventory_positions").select("id,item_id,card_name,scryfall_id,set_code,collector_number,finish,condition,quantity,location_id,status,created_at").eq("batch_id", batchId).eq("user_id", auth.user.id).order("card_name", { ascending: true }),
