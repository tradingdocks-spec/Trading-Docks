...
export default async function ShowcaseSettingsPage() {
  ...
  const { data: profile } = preference?.active_workspace_id ? await supabase.from("showcase_profiles").select("*").eq("workspace_id", preference.active_workspace_id).maybeSingle() : { data: null };
