...
export async function GET(request: Request) {
  ...
  let query = admin.from("marketing_assets").select("id,name,slug,asset_type,storage_path,mime_type,width,height,tags,feature_ids,game_ids,approved_for_marketing,approval_status,source,source_url,license_notes,alt_text,product_display_allowed,marketing_use_approved,archived_at,created_at,updated_at").order("updated_at", { ascending: false }).limit(300);
