...
export async function POST(request: Request) {
  ...
  const metadata = await admin.from("marketing_assets").insert({ name: file.name.replace(/\.[^.]+$/, "").slice(0, 160), asset_type: form.get("assetType") || "other", storage_path: path, mime_type: file.type, width: dimensions(bytes, file.type).width, height: dimensions(bytes, file.type).height, tags: String(form.get("tags") || "").split(",").map((tag) => tag.trim()).filter(Boolean).slice(0, 30), source: String(form.get("source") || "admin_uploaded").slice(0, 120), source_url: String(form.get("sourceUrl") || "").slice(0, 500) || null, license_notes: String(form.get("licenseNotes") || "").slice(0, 2000), alt_text: String(form.get("altText") || "").slice(0, 300), product_display_allowed: form.get("productDisplayAllowed") === "true", marketing_use_approved: false, approval_status: "draft", created_by: actor.user.id }).select("id,name,asset_type,width,height,approval_status,storage_path").single();
