...
const ASSET_TYPES = new Set(["logo", "product_screenshot", "feature_screenshot", "card_image", "background", "texture", "device_mockup", "icon", "campaign_artwork", "social_export", "email_export", "other"]);
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (typeof body.assetType === "string" && !ASSET_TYPES.has(body.assetType)) return NextResponse.json({ error: "Unsupported asset type." }, { status: 400 });
  ...
  if (typeof body.assetType === "string") updates.asset_type = body.assetType;
  if (Array.isArray(body.featureIds)) updates.feature_ids = body.featureIds.filter((value): value is string => typeof value === "string");
  ...
  if ("screenshotRole" in body && body.screenshotRole === null) updates.screenshot_role = null;
  ...
  const { data, error } = await admin.from("marketing_assets").update(updates).eq("id", id).select("id,name,asset_type,feature_ids,width,height,tags,approval_status,source,source_url,license_notes,alt_text,marketing_use_approved,brand_role,screenshot_role,focal_x,focal_y,safe_crop,preferred_aspect_ratios,archived_at,updated_at").single();
