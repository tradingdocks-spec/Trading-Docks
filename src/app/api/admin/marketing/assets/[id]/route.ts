...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await admin.from("marketing_assets").update(updates).eq("id", id).select("id,name,asset_type,width,height,tags,approval_status,source,source_url,license_notes,alt_text,marketing_use_approved,archived_at,updated_at").single();
