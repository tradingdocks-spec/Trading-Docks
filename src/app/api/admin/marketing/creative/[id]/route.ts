...
async function loadCreative(id: string) {
  return createAdminClient().from("marketing_creatives").select("id,campaign_id,brief_id,name,status,platform,variant_key,width,height,copy_payload,render_spec,quality_issues,asset_ids,parent_creative_id,variant_group_id,created_at,marketing_creative_briefs(prospect_id,brief)").eq("id", id).maybeSingle();
}
