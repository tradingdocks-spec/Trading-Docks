...
import { buildRenderSpec, renderCreativeSvg, type CreativeRenderSpec } from "@/lib/marketing/creative-renderer";
...
export async function POST(request: Request) {
  ...
  return new NextResponse(output as unknown as BodyInit, { headers: { "Content-Type": `image/${format}`, "Content-Disposition": `attachment; filename="trading-docks-${spec.featureName.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-${spec.platform}-v1.${format}"`, "Cache-Control": "no-store" } });
}
