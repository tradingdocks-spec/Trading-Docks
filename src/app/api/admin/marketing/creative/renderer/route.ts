...
import { runBrandQualityChecks } from "@/lib/marketing/brand-system";
...
export async function POST(request: Request) {
  ...
  const body = await request.json().catch(() => null) as { action?: string; spec?: Partial<CreativeRenderSpec>; campaignId?: string; briefId?: string; creativeId?: string; assetIds?: string[]; conceptDirection?: "product" | "transformation" | "editorial"; campaignVisualFamilyId?: string; brandProfileVersion?: number } | null;
  ...
  const qualityReview = runBrandQualityChecks({ headline: spec.headline, subheadline: spec.subheadline, cta: spec.cta, platform: spec.platform, productAssetApproved, logoAssetApproved, hasFeatureCopy: Boolean(spec.featureName.trim()), hasOverflow: false });
