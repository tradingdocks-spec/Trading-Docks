...
export class TradingDocksLocalScannerProvider implements ScannerProvider {
  ...
  async unpair() { try { await this.call("/v1/unpair", {}); } finally { await this.storage.clear(); this.credential = undefined; await this.disconnect(); } }
  async detect() {
  ...
  async capture(signal?: AbortSignal) {
    ...
    try {
      ...
      while (Date.now() < deadline) {
        ...
        const result = await this.call<{ status: string; error?: string; image?: string; mimeType: string; width: number; height: number }>(`/v1/capture/${started.captureId}`);
