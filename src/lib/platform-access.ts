...
export const normalizeAccountType = normalizeMembershipTier;

export { normalizeBillingStatus, normalizeMembershipTier };
...
export const CAPABILITY_REGISTRY: Record<PlatformCapability, CapabilityRequirement> = {
  ...
  "scanner.use": {
    capability: "scanner.use",
    label: "Use scanner",
    minimumTier: "free",
    entitlement: "card-scanner",
  },
  "scanner.advanced": { capability: "scanner.advanced", label: "Advanced scanner tools", minimumTier: "collector" },
  "binder.manage": {
    capability: "binder.manage",
    label: "Manage Trade Binder",
    minimumTier: "collector",
    entitlement: "trade-binder",
  },
  "wishlist.manage": {
    capability: "wishlist.manage",
  ...
  "buying.manage": {
    ...
    label: "Manage buying workflows",
    minimumTier: "seller",
    entitlement: "deal-desk",
  },
  "orders.manage": { capability: "orders.manage", label: "Manage orders", minimumTier: "seller" },
  "marketplaces.manage": {
    capability: "marketplaces.manage",
  ...
  "businessIntelligence.view": {
    ...
    label: "View business intelligence",
    minimumTier: "store",
  },
...
export function hasPlatformRole(
  role: PlatformRole,
  minimum: Exclude<PlatformRole, "user">,
) {
...
export function resolvePlatformAccessContext(input: PlatformAccessInput = {}): PlatformAccessContext {
  ...
  const accountType = normalizeMembershipTier(input.accountType);
  const billingStatus = normalizeBillingStatus(
    input.billingStatus ?? (input.billingPlan ? "active" : "free"),
  );
  const now = input.now ?? new Date();
  const paidBillingIsCurrent = billingStatusAllowsPaidEntitlements({
    status: billingStatus,
    periodEnd: input.billingPeriodEnd ?? null,
    now,
  });
  ...
  const canAccessCommandCenter = platformRole !== "user" && !suspended;
  if (canAccessCommandCenter) entitlements.push("admin.command-center");
...
export function hasCapability(
  ...
  if (
    ...
    MEMBERSHIP_TIER_RANK[access.membershipTier] < MEMBERSHIP_TIER_RANK[requirement.minimumTier]
  ) {
    return false;
  }
  if (requirement.accountTypes && !requirement.accountTypes.includes(access.accountType)) {
    return false;
  }
...
export function clientAccessFromTier(
  tier: unknown,
  options: Partial<ClientSafePlatformAccess> = {},
): ClientSafePlatformAccess {
  const membershipTier = normalizeMembershipTier(tier);
  return {
    authenticated: options.authenticated ?? true,
    platformRole: options.platformRole ?? "user",
    accountType: options.accountType ?? normalizeMembershipTier(tier),
    membershipTier,
