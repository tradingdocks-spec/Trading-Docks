...
export function buildCanonicalCaptureMetadata(request: CanonicalCaptureRequest, featureId: string, version = "v1"): CanonicalCaptureMetadata {
  ...
  const cropMode = request.state === "workflow" ? "workflow" : request.state === "review" ? "review" : request.state === "inventory-result" ? "inventory_result" : "product_ui";
  return { name: `${feature.name} · ${fixture.label}`, slug, assetType: "product_screenshot", storagePath: `canonical/${feature.slug}/${request.state}/${request.viewport}-${version}.png`, source: "canonical_product_capture", featureIds: [featureId], screenshotRole: request.role, cropMode, approvalStatus: "approved", marketingUseApproved: true, captureVersion: version, safeCrop: true, width: request.width, height: request.height };
}
