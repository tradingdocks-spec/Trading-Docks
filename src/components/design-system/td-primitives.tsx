...
const textVariant: Record<TDTextVariant, string> = {
  display: "text-[2.375rem] leading-[1.1] font-bold tracking-normal",
  heading: "text-[1.625rem] leading-tight font-bold tracking-normal",
  title: "text-lg leading-snug font-bold tracking-normal",
  ...
  label: "text-[11px] leading-4 font-bold uppercase tracking-[0.1em]",
};
...
export function TDButton({
  ...
  return (
    <button
      ...
      disabled={isDisabled}
      className={cn(
        "inline-flex shrink-0 items-center justify-center gap-2 rounded-[var(--td-radius-md)] border font-semibold transition outline-none focus-visible:ring-2 focus-visible:ring-[var(--td-border-focus)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--td-background-primary)] disabled:pointer-events-none disabled:opacity-55",
...
export function TDInput({
  ...
  return (
    <div className={cn("space-y-2", containerClassName)}>
      {label ? (
        <label htmlFor={inputId} className="block text-[11px] font-bold uppercase tracking-[0.1em] text-[var(--td-text-muted)]">
          {label}
        </label>
      ) : null}
      <input
...
export function TDBadge({ className, tone = "neutral", ...props }: ComponentProps<"span"> & { tone?: TDBadgeTone }) {
  return (
    <span
      data-slot="td-badge"
      className={cn("inline-flex w-fit items-center rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.08em]", badgeTone[tone], className)}
      {...props}
    />
  );
}
