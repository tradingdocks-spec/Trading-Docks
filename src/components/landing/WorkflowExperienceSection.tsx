...
export function WorkflowExperienceSection() {
  return (
    <section
      data-td-reveal
      className="relative z-10 bg-[#020912] px-5 py-16 text-white sm:px-8 sm:py-20 lg:px-12"
    >
      <div className="mx-auto max-w-[1480px]">
        <div className="grid min-w-0 gap-10 lg:grid-cols-[360px_1fr]">
          <div className="min-w-0">
            <p className="text-sm font-medium text-cyan-200">Workflow trace</p>
          ...
          <div className="min-w-0 border-y border-white/[0.08]">
            {LIFECYCLE_TRACE.map(([number, label, copy]) => (
              <div
                key={label}
                className="grid gap-4 border-b border-white/[0.06] py-5 last:border-b-0 sm:grid-cols-[70px_170px_1fr]"
              >
