    ...
    >
      <div className="mx-auto grid max-w-[1480px] min-w-0 gap-10 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="min-w-0">
          <p className="text-sm font-medium text-td-accent-text">Platform authority</p>
        ...
        <div className="min-w-0">
          <div className="grid border-y border-td-ink/[0.08] md:grid-cols-2">
            {PRODUCT_AUTHORITIES.map((item) => (
              <article
                key={item.name}
                className="border-b border-td-ink/[0.08] py-5 pr-5 odd:md:border-r even:md:pl-5 md:[&:nth-last-child(-n+2)]:border-b-0"
              >
                <p className="text-xs text-td-muted">{item.owner}</p>
                <h3 className="mt-2 text-xl font-semibold tracking-[-0.025em] text-td-primary">
                  {item.name}
                </h3>
                <p className="mt-3 text-sm leading-6 text-td-muted">{item.detail}</p>
              </article>
            ))}
          </div>
            ...
            <table className="w-full min-w-[720px] border-collapse text-left">
              ...
              <tbody>
                {DECISION_ROWS.map(([question, authority, reason]) => (
                  <tr key={question} className="border-b border-td-ink/[0.055] last:border-b-0">
                    <td className="py-4 pr-6 text-sm font-medium text-td-secondary">{question}</td>
                    <td className="px-4 py-4 text-sm text-td-muted">{authority}</td>
                    <td className="px-4 py-4 text-sm text-td-muted">{reason}</td>
                  </tr>
                ))}
              </tbody>
            </table>
