      ...
      <div className="mx-auto max-w-[1480px]">
        <div className="grid min-w-0 gap-10 lg:grid-cols-[360px_1fr]">
          <div className="min-w-0">
            <p className="text-sm font-medium text-cyan-200">Plan progression</p>
          ...
          <div className="min-w-0 border-y border-white/[0.08]">
            {PLAN_STAGES.map((stage, index) => {
              const plan = MEMBERSHIP_PLANS[stage.tier];
              return (
                <div
                  key={stage.tier}
                  className="grid gap-4 border-b border-white/[0.06] py-5 last:border-b-0 md:grid-cols-[64px_150px_120px_1fr]"
                >
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-600">{stage.motion}</p>
                  </div>
                  <p className="text-2xl font-semibold tracking-[-0.04em] text-white">
                    {price(stage.tier)}
                  </p>
                  <p className="text-sm leading-6 text-slate-500">{stage.operationalQuestion}</p>
                </div>
              );
            })}
          </div>
