      ...
      <div className="mx-auto max-w-[1480px]">
        <div className="grid min-w-0 gap-10 lg:grid-cols-[360px_1fr]">
          <div className="min-w-0">
            <p className="text-sm font-medium text-td-accent-text">Workspace fit</p>
          ...
          <div className="min-w-0">
            <div className="grid border-y border-td-ink/[0.08] md:grid-cols-3">
              {PERSONAS.map((persona) => {
                ...
                return (
                  <button
                    ...
                    className={[
                      "border-b border-td-ink/[0.08] py-5 text-left transition md:border-b-0 md:border-r md:px-5 md:last:border-r-0",
                      selected ? "text-td-primary" : "text-td-muted hover:text-td-primary",
                    ].join(" ")}
                  >
                    <span className="text-xs text-td-muted">{item.disclosure}</span>
                    <span className="mt-2 block text-2xl font-semibold tracking-[-0.035em]">
                      {item.plan}
                    </span>
                    <span className="mt-2 block text-sm leading-6 text-td-muted">
                      {item.tagline}
                    </span>
                  </button>
                );
            ...
            <div className="grid gap-8 border-b border-td-ink/[0.08] py-8 lg:grid-cols-[1fr_320px]">
              <div>
                <div className="grid grid-cols-2 border-y border-td-ink/[0.08]">
                  {demo.metricLabels.map((label, index) => (
                    <div
                      key={label}
                      className="border-b border-td-ink/[0.06] py-4 pr-4 odd:border-r even:pl-4 [&:nth-last-child(-n+2)]:border-b-0"
                    >
                      <p className="text-xs text-td-muted">{label}</p>
                      <p className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-td-primary">
              ...
              <aside>
                <p className="text-sm font-semibold text-td-primary">{demo.plan} navigation</p>
                <div className="mt-4 grid gap-3">
                  {demo.nav.map((item, index) => (
                    <div key={item} className="flex justify-between border-b border-td-ink/[0.06] pb-3">
                      <span className="text-sm text-td-secondary">{item}</span>
            ...
            <div className="grid border-b border-td-ink/[0.08] md:grid-cols-4">
              {workflow.map((step, index) => (
                <div
                  key={step.label}
                  className="border-b border-td-ink/[0.08] py-5 md:border-b-0 md:border-r md:px-5 md:last:border-r-0"
                >
                  <p className="text-xs text-td-muted">{String(index + 1).padStart(2, "0")}</p>
                  <h3 className="mt-3 text-lg font-semibold text-td-primary">{step.label}</h3>
                  <p className="mt-2 text-sm text-td-secondary">{step.action}</p>
                  <p className="mt-3 text-xs leading-5 text-td-muted">{step.evidence}</p>
                </div>
              ))}
            </div>
          </div>
