  ...
  return (
    <main className="min-h-screen bg-td-canvas px-4 py-5 text-td-primary sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[1480px]">
        {loadError ? (
          <div className="mb-5 rounded-2xl border border-td-danger/20 bg-td-danger/10 px-5 py-4 text-sm text-td-danger">
            {loadError}
          </div>
        ) : null}

        <header className="relative overflow-hidden rounded-[28px] border border-td-accent/[0.10] bg-td-surface px-5 py-5 shadow-[0_24px_90px_rgb(var(--td-shadow-rgb)/calc(0.22*var(--td-shadow-strength)))] sm:px-7 sm:py-6">
          <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-td-accent/30 to-transparent" />
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-4xl">
              <div className="flex flex-wrap items-center gap-3">
                <span className="flex h-10 w-10 items-center justify-center rounded-2xl border border-td-accent/[0.14] bg-td-accent/[0.055] text-td-accent-text">
            ...
            <div className="flex flex-wrap gap-2.5">
              <Link
                ...
                className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-td-accent px-4 text-[12px] font-bold text-td-on-accent shadow-[0_16px_36px_rgb(var(--td-accent-rgb)/.16)] transition hover:bg-td-accent-hover focus:outline-none focus:ring-2 focus:ring-td-accent"
              >
              ...
              <Link
                ...
                className="inline-flex h-11 items-center justify-center rounded-xl px-3 text-[12px] font-semibold text-td-muted transition hover:bg-td-ink/[0.025] hover:text-td-primary focus:outline-none focus:ring-2 focus:ring-td-accent/30"
              >
                Paste decklist
              </Link>
            </div>
        ...
        <section className="mt-5 rounded-[26px] bg-td-surface p-5 shadow-[inset_0_1px_0_rgb(var(--td-ink-rgb)/.045),0_20px_70px_rgb(var(--td-shadow-rgb)/calc(.20*var(--td-shadow-strength)))]">
          <div className="grid gap-5 lg:grid-cols-[1.1fr_1.7fr] lg:items-center">
            <div>
              ...
              <div className="mt-2 flex flex-wrap items-end gap-x-3 gap-y-1">
                <span className="text-4xl font-semibold tracking-[-0.055em] text-td-primary">
                  {summary.deckCountLabel}
                </span>
                <span className="pb-1 text-sm text-td-muted">
                  {summary.limitLabel}
                </span>
              </div>
        ...
        <section className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.55fr)_minmax(340px,0.65fr)]">
          <div className="rounded-[28px] bg-td-surface p-4 shadow-[inset_0_1px_0_rgb(var(--td-ink-rgb)/.045),0_22px_80px_rgb(var(--td-shadow-rgb)/calc(.22*var(--td-shadow-strength)))] sm:p-5">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
              ...
              <div className="grid gap-2 sm:grid-cols-[minmax(150px,0.7fr)_minmax(150px,0.7fr)_minmax(220px,1fr)] xl:min-w-[640px]">
                ...
                <label className="flex h-11 min-w-0 items-center gap-2 rounded-xl bg-black/[0.16] px-3 ring-1 ring-td-ink/[0.07] transition focus-within:ring-td-accent/35">
                  <Search className="h-4 w-4 shrink-0 text-td-muted" />
                  <input
                    value={searchQuery}
                    onChange={(event) => setSearchQuery(event.target.value)}
                    placeholder="Search decks"
                    className="min-w-0 flex-1 bg-transparent text-[12px] text-td-secondary outline-none placeholder:text-td-muted"
                  />
                </label>
              </div>
            </div>
...
function DeckCard({ deck, onRename }: { deck: DeckRecord; onRename: () => void }) {
  ...
  return (
    <article className="group relative overflow-hidden rounded-[24px] bg-td-surface shadow-[inset_0_1px_0_rgb(var(--td-ink-rgb)/.05),0_18px_52px_rgb(var(--td-shadow-rgb)/calc(.24*var(--td-shadow-strength)))] transition duration-300 hover:-translate-y-0.5 hover:bg-td-surface">
      <DeckCardHoverPreview card={commanderCard}>
        {({ previewProps }) => (
          <div className="relative h-32 overflow-hidden bg-td-canvas" {...previewProps}>
            {image ? (
              ...
              <div className="flex h-full items-center justify-center bg-[linear-gradient(135deg,rgb(var(--td-accent-rgb)/.12),rgb(var(--td-surface-rgb)/.18))]">
                <ManaPips colors={colors} size="lg" />
              </div>
            )}
            <div className="absolute inset-0 bg-gradient-to-t from-td-surface via-td-surface/35 to-transparent" />
            <div className="absolute left-4 top-4 flex items-center gap-2">
              <ManaPips colors={colors} />
              <span className="rounded-full bg-black/45 px-2.5 py-1 text-[11px] font-bold uppercase tracking-[0.12em] text-td-primary backdrop-blur">
                {deck.format}
              </span>
            </div>
            <button
              type="button"
              onClick={onRename}
              className="absolute right-3 top-3 flex h-9 w-9 items-center justify-center rounded-xl bg-black/45 text-td-secondary backdrop-blur transition hover:bg-td-accent/15 hover:text-td-accent-text focus:outline-none focus:ring-2 focus:ring-td-accent/50"
      ...
      <div className="p-4">
        <div className="min-h-[82px]">
          <h3 className="line-clamp-2 text-[19px] font-semibold leading-6 tracking-[-0.035em] text-td-primary">
            {deck.name}
          </h3>
          <p className="mt-1 line-clamp-1 text-[13px] font-medium text-td-secondary">
        ...
        {isCommander ? (
          commanderBracket ? (
            <div className="mt-3 rounded-2xl bg-td-violet/[0.055] p-3 ring-1 ring-td-violet/[0.11]">
              <div className="flex items-center justify-between gap-3">
                <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-td-violet/80">
                  Commander bracket
                </p>
                <span className="rounded-full bg-td-violet px-2 py-0.5 text-[11px] font-bold text-td-on-accent">
                  Bracket {commanderBracket.bracket}
                </span>
              </div>
              <p className="mt-1 text-[12px] font-semibold text-td-primary">
                {commanderBracket.name}
              </p>
              {commanderBracket.reasons.length ? (
                <p className="mt-1 line-clamp-2 text-[11px] leading-4 text-td-violet/62">
                  {commanderBracket.reasons[0]}
                </p>
              ) : null}
              <Link
                href={`/dashboard/deck-vault/decks/${deck.id}?view=analytics`}
                className="mt-2 inline-flex text-[11px] font-semibold text-td-violet transition hover:text-td-violet"
              >
                View bracket analysis
              </Link>
            </div>
          ) : (
            <div className="mt-3 rounded-2xl bg-td-violet/[0.035] p-3 ring-1 ring-td-violet/[0.08]">
              <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-td-violet/70">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <Link
            ...
            className="inline-flex h-10 flex-1 items-center justify-center gap-2 rounded-xl bg-td-accent px-3 text-[12px] font-bold text-td-on-accent transition hover:bg-td-accent-hover focus:outline-none focus:ring-2 focus:ring-td-accent"
          >
            Open deck
          ...
          <Link
            ...
            className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-td-ink/[0.08] px-3 text-[12px] font-semibold text-td-secondary transition hover:border-td-violet/25 hover:bg-td-violet/[0.045] hover:text-td-violet focus:outline-none focus:ring-2 focus:ring-td-violet/40"
          >
            Analyze
          </Link>
        </div>
      ...
      {mounted && card && imageSource && position
        ? createPortal(
            <div
              ...
              className="pointer-events-none fixed z-[500] overflow-hidden rounded-[22px] border border-td-accent/[0.16] bg-td-surface/98 p-3 shadow-[0_28px_90px_rgb(var(--td-shadow-rgb)/calc(0.72*var(--td-shadow-strength))),0_0_28px_rgb(var(--td-accent-rgb)/0.09)] backdrop-blur-xl"
            ...
            >
              ...
              />
              <p className="mt-3 line-clamp-2 text-[14px] font-semibold leading-5 text-td-primary">
                {card.name}
              </p>
              <div className="mt-2 flex items-center justify-between gap-3 text-[11px]">
                <span className="text-td-muted">MV {card.manaValue}</span>
...
function AiDeckReviewPanel({ hasDecks, firstDeck }: { hasDecks: boolean; firstDeck?: DeckRecord }) {
  return (
    <section className="rounded-[26px] bg-td-surface p-5 shadow-[inset_0_1px_0_rgb(var(--td-ink-rgb)/.045),0_20px_70px_rgb(var(--td-shadow-rgb)/calc(.20*var(--td-shadow-strength)))]">
      <div className="flex items-start gap-3">
        <span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-td-violet/[0.08] text-td-violet">
      ...
      <div className="mt-5 rounded-2xl bg-black/[0.13] p-4">
        ...
        <Link
          ...
          className="mt-4 inline-flex h-10 items-center gap-2 rounded-xl bg-td-violet px-3.5 text-[12px] font-bold text-td-on-accent transition hover:bg-td-violet focus:outline-none focus:ring-2 focus:ring-td-violet"
        >
  ...
  return (
    <section className="rounded-[26px] bg-td-surface p-5 shadow-[inset_0_1px_0_rgb(var(--td-ink-rgb)/.045),0_20px_70px_rgb(var(--td-shadow-rgb)/calc(.20*var(--td-shadow-strength)))]">
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.15em] text-td-success/80">
            Collection connection
          </p>
          <h2 className="mt-1.5 text-lg font-semibold text-td-primary">
            {hasDecks ? `${summary.averageCoverage}% average coverage` : "Coverage starts in Collection"}
          </h2>
        </div>
        <BarChart3 className="h-5 w-5 text-td-success" />
      </div>
      <p className="mt-3 text-[12px] leading-5 text-td-muted">
        {hasDecks
      ...
      <div className="mt-4 h-2 overflow-hidden rounded-full bg-td-ink/[0.05]">
        <div
          className="h-full rounded-full bg-td-success"
...
function CollectorLoopPanel() {
  return (
    <section className="rounded-[26px] bg-td-surface p-5 shadow-[inset_0_1px_0_rgb(var(--td-ink-rgb)/.045),0_20px_70px_rgb(var(--td-shadow-rgb)/calc(.20*var(--td-shadow-strength)))]">
      <p className="text-[11px] font-bold uppercase tracking-[0.15em] text-td-accent-text">First deck workflow</p>
      <div className="mt-4 grid gap-2">
        {["Import deck", "Compare to collection", "Analyze", "Upgrade"].map((item, index) => (
          <div key={item} className="flex items-center gap-3 rounded-xl bg-td-ink/[0.025] px-3 py-2.5 text-[12px] text-td-secondary">
            <span className="flex h-6 w-6 items-center justify-center rounded-full bg-td-accent/[0.08] text-[11px] font-bold text-td-accent-text">
              {index + 1}
            </span>
            {item}
          </div>
...
function ControlSelect({
  ...
  return (
    <label className="relative flex h-11 min-w-0 items-center rounded-xl bg-black/[0.16] px-3 ring-1 ring-td-ink/[0.07] transition focus-within:ring-td-accent/35">
      <span className="sr-only">{label}</span>
      <select
        value={value}
        onChange={(event) => onChange(event.target.value)}
        className="w-full appearance-none bg-transparent pr-7 text-[12px] font-semibold text-td-secondary outline-none"
      >
        {children}
      </select>
      <ChevronDown className="pointer-events-none absolute right-3 h-3.5 w-3.5 text-td-muted" />
    </label>
  );
}
...
function EmptyPanel({
  ...
  return (
    <div className="col-span-full flex min-h-[310px] flex-col items-center justify-center rounded-[24px] bg-black/[0.12] px-6 py-12 text-center">
      <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-td-accent/[0.08] text-td-accent-text">
        <LibraryBig className="h-5 w-5" />
      </span>
      <p className="mt-4 text-base font-semibold text-td-primary">{title}</p>
      <p className="mt-2 max-w-md text-[12px] leading-5 text-td-muted">{body}</p>
      <Link href={actionHref} className="mt-5 inline-flex h-10 items-center gap-2 rounded-xl bg-td-accent px-4 text-[12px] font-bold text-td-on-accent">
        {actionLabel}
        <ArrowRight className="h-3.5 w-3.5" />
      </Link>
    </div>
...
function SummaryStat({ label, value, detail }: { label: string; value: string; detail: string }) {
  return (
    <div className="rounded-2xl bg-td-ink/[0.028] p-4">
      <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-td-muted">{label}</p>
      <p className="mt-2 text-xl font-semibold tracking-[-0.03em] text-td-primary">{value}</p>
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl bg-td-ink/[0.025] p-3">
      <p className="text-[11px] font-semibold uppercase tracking-[0.09em] text-td-muted">{label}</p>
      <p className="mt-1 truncate text-[12px] font-semibold text-td-primary">{value}</p>
    </div>
  );
}
