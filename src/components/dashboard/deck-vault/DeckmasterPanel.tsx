...
export function DeckmasterPanel({ deck, collection, onApply, mobileOpen, onMobileClose }: {
  ...
  return (
    <aside aria-label="Deckmaster AI Deck Assistant" className={[
      "border-td-ink/[0.08] bg-td-surface shadow-[0_24px_80px_rgb(var(--td-shadow-rgb)/calc(.3*var(--td-shadow-strength)))]",
      ...
      <div className="flex items-start justify-between gap-3 border-b border-td-ink/[0.07] pb-4">
        <div><div className="flex items-center gap-2 text-td-accent-text"><Sparkles className="h-4 w-4" /><span className="text-[11px] font-semibold uppercase tracking-[0.18em]">AI Deck Assistant</span></div><h2 className="mt-1 text-xl font-semibold text-td-primary">Deckmaster</h2><p className="mt-1 text-xs text-td-muted">Working on {context.deckName} · {context.format}</p></div>
        <button type="button" onClick={onMobileClose} className="flex h-10 w-10 items-center justify-center rounded-xl border border-td-ink/[0.08] text-td-secondary lg:hidden" aria-label="Close Deckmaster"><X className="h-4 w-4" /></button>
      </div>

      <p className="mt-4 text-sm leading-6 text-td-secondary" role="status">{message}</p>
      <div className="mt-5 grid grid-cols-2 gap-2">
        {QUICK_ACTIONS.map((action) => <button key={action} type="button" onClick={() => void submit(action)} className="min-h-11 rounded-xl border border-td-ink/[0.07] bg-td-ink/[0.025] px-3 py-2 text-left text-xs font-medium text-td-secondary transition hover:border-td-accent/25 hover:bg-td-accent/[0.05] hover:text-td-primary">{action}</button>)}
      </div>
      ...
      <form onSubmit={(event) => { event.preventDefault(); void submit(); }} className="mt-5 border-t border-td-ink/[0.07] pt-4">
        <label htmlFor="deckmaster-input" className="sr-only">Ask Deckmaster about this deck</label>
        <textarea id="deckmaster-input" value={input} onChange={(event) => setInput(event.target.value)} placeholder="Ask Deckmaster about this deck..." rows={3} className="w-full resize-none rounded-xl border border-td-ink/[0.09] bg-black/20 px-3 py-3 text-sm text-td-primary outline-none placeholder:text-td-muted focus:border-td-accent/40" />
        <button disabled={busy || !input.trim()} className="mt-2 inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-td-accent text-xs font-semibold text-td-on-accent disabled:opacity-40">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <ArrowRight className="h-4 w-4" />} Ask Deckmaster</button>
      </form>
    </aside>
  );
}
...
function ProposalReview({ proposal, onApply, onCancel }: { proposal: DeckChangeProposal; onApply: () => void; onCancel: () => void }) {
  ...
  return <section className="mt-5 rounded-2xl border border-td-accent/15 bg-td-accent/[0.035] p-4">
    <p className="text-sm font-semibold text-td-primary">Deckmaster recommends {count} {count === 1 ? "change" : "changes"}</p>
    <details open className="mt-3">
      <summary className="cursor-pointer text-xs font-semibold text-td-accent-text">Review Changes</summary>
      {proposal.additions.length ? <ChangeList label="Add" changes={proposal.additions} tone="text-td-success" /> : null}
      {proposal.removals.length ? <ChangeList label="Remove" changes={proposal.removals} tone="text-td-danger" /> : null}
      {proposal.swaps.map((swap) => <p key={`${swap.remove.card.id}-${swap.add.card.id}`} className="mt-3 text-xs text-td-secondary">Swap {swap.remove.card.name} → {swap.add.card.name}</p>)}
    </details>
    <dl className="mt-4 space-y-2 border-t border-td-ink/[0.07] pt-3 text-xs"><Row label="Estimated price change" value={`${proposal.pricingImpact >= 0 ? "+" : "−"}$${Math.abs(proposal.pricingImpact).toFixed(2)}`} /><Row label="Owned / missing change" value={`${signed(proposal.ownershipImpact.owned)} owned · ${signed(proposal.ownershipImpact.missing)} missing`} /></dl>
    <div className={`mt-3 flex items-start gap-2 rounded-xl p-3 text-xs ${proposal.legality.canApply ? "bg-td-success/10 text-td-success" : "bg-td-danger/10 text-td-danger"}`}><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" /><span>{proposal.legality.valid ? "Legal complete-deck state" : proposal.legality.canApply ? "Safe for this in-progress deck; complete-format validation is still pending." : proposal.legality.introducedIssues.map((issue) => issue.message).join(" ")}</span></div>
    <div className="mt-4 grid grid-cols-2 gap-2"><button type="button" onClick={onCancel} className="h-10 rounded-xl border border-td-ink/[0.09] text-xs font-semibold text-td-secondary">Cancel</button><button type="button" onClick={onApply} disabled={!proposal.legality.canApply} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-td-accent text-xs font-semibold text-td-on-accent disabled:cursor-not-allowed disabled:opacity-40"><CheckCircle2 className="h-4 w-4" />Apply All</button></div>
  </section>;
}

function ChangeList({ label, changes, tone }: { label: string; changes: Array<{ card: DeckCard; quantity: number }>; tone: string }) { return <div className="mt-3"><p className={`text-[11px] font-semibold uppercase tracking-[0.14em] ${tone}`}>{label}</p>{changes.map((change) => <div key={`${label}-${change.card.id}`} className="mt-2 flex items-center gap-3"><div className="h-12 w-9 overflow-hidden rounded-md bg-td-ink/5">{change.card.image ? <img src={change.card.image} alt="" className="h-full w-full object-cover" /> : null}</div><span className="text-xs text-td-primary">{change.quantity}× {change.card.name}</span></div>)}</div>; }
function Row({ label, value }: { label: string; value: string }) { return <div className="flex justify-between gap-3"><dt className="text-td-muted">{label}</dt><dd className="text-right text-td-primary">{value}</dd></div>; }
