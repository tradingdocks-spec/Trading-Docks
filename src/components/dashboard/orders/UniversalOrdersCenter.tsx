...
export function UniversalOrdersCenter({
  ...
  return <WorkspaceFrame><div className="space-y-4 sm:space-y-5">
    <header className="relative overflow-hidden rounded-[20px] border border-white/[0.08] bg-[linear-gradient(135deg,rgba(7,26,38,.96),rgba(4,15,23,.98))] px-5 py-4 shadow-[0_18px_55px_rgba(0,0,0,.2)] sm:px-6 sm:py-5">
      <div className="relative flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex flex-wrap gap-2"><Link href="/dashboard/marketplaces" className="td-button-secondary px-4"><Link2 className="h-4 w-4 text-cyan-300" /> Manage channels</Link><button type="button" onClick={exportCsv} className="td-button-primary px-4"><Download className="h-4 w-4" /> Export CSV</button></div>
      </div>
    </header>
    ...
    <section className="overflow-hidden rounded-[20px] border border-white/[0.075] bg-[rgba(6,18,27,.86)] shadow-[0_16px_44px_rgba(0,0,0,.15)]">
...
function OrderRow({ order, checked, expanded, onCheck, onExpand }: { order: OrderRecord; checked: boolean; expanded: boolean; onCheck: () => void; onExpand: () => void }) {
  ...
  return <div className="border-b border-white/[0.052] last:border-0"><div className="grid grid-cols-[38px_1.25fr_.72fr_.8fr_.75fr_.72fr_.72fr_42px] items-center gap-4 px-5 py-3.5 transition hover:bg-white/[0.022]"><input type="checkbox" aria-label={`Select order ${order.external_order_id}`} checked={checked} onChange={onCheck} className="accent-cyan-300" /><button type="button" onClick={onExpand} className="min-w-0 text-left"><div className="flex items-center gap-2"><span className="truncate text-sm font-semibold text-white">#{order.external_order_id}</span>{unmatched ? <span title={`${unmatched} unmatched line items`} className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-amber-300/[0.08]"><AlertTriangle className="h-3 w-3 text-amber-300" /></span> : null}</div><p className="mt-1 truncate text-[0.8rem] text-slate-500">{order.buyer_alias || "Marketplace customer"} · {formatDate(order.ordered_at)}</p></button><ChannelBadge name={order.marketplace_id} /><StatusBadge status={state} /><div><p className="text-[13px] font-semibold text-slate-300">{units} unit{units === 1 ? "" : "s"}</p><p className="mt-1 text-[0.8rem] text-slate-500">{items.length} line item{items.length === 1 ? "" : "s"}</p></div><div><p className="text-sm font-semibold text-white">{money.format(Number(order.total ?? 0))}</p><p className="mt-1 text-[0.8rem] text-slate-500">incl. shipping</p></div><div><p className={`text-sm font-semibold ${Number(order.net_profit ?? 0) > 0 ? "text-emerald-300" : "text-slate-400"}`}>{money.format(Number(order.net_profit ?? 0))}</p><p className="mt-1 text-[0.8rem] text-slate-500">after costs</p></div><button type="button" onClick={onExpand} aria-label={`${expanded ? "Collapse" : "Expand"} order ${order.external_order_id}`} className="grid h-8 w-8 place-items-center rounded-lg text-slate-600 hover:bg-white/[0.05] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/30"><MoreHorizontal className="h-4 w-4" /></button></div>
    {expanded ? <div className="border-t border-white/[0.05] bg-black/[0.12] px-5 py-4"><div className="grid gap-4 lg:grid-cols-[1fr_320px]"><div><div className="mb-2 flex items-center justify-between gap-3"><p className="text-xs font-bold uppercase tracking-[.14em] text-slate-600">Line items</p><Link href={`/dashboard/orders/${encodeURIComponent(order.id)}/pick`} className="td-button-primary px-3 py-2 text-xs">Start Pick Mode</Link></div>{items.length ? <div className="space-y-2">{items.map((item) => <div key={item.id} className="flex items-center gap-3 rounded-xl border border-white/[0.055] bg-white/[0.02] p-2.5"><div className="grid h-12 w-10 shrink-0 place-items-center overflow-hidden rounded-lg bg-white/[0.04]">{item.image_url ? <img src={item.image_url} alt="" loading="lazy" className="h-full w-full object-cover" /> : <PackageOpen className="h-4 w-4 text-slate-700" />}</div><div className="min-w-0 flex-1"><p className="truncate text-[13px] font-semibold text-slate-200">{item.quantity}× {item.title}</p><p className="mt-1 truncate text-xs text-slate-500">{[item.condition, item.language, item.finish, item.external_sku].filter(Boolean).join(" · ") || "Details unavailable"}</p>{item.physical_location ? <p className="mt-2 text-xs font-bold uppercase tracking-[.12em] text-cyan-200">{item.physical_location.pathLabel}</p> : <p className="mt-2 text-xs font-bold uppercase tracking-[.12em] text-amber-300">LOCATION UNKNOWN</p>}</div><div className="text-right"><p className="text-[13px] font-semibold text-white">{money.format(Number(item.unit_price ?? 0) * item.quantity)}</p><p className={`mt-1 text-xs font-bold uppercase ${item.match_status === "matched" ? "text-emerald-300" : "text-amber-300"}`}>{item.match_status ?? "unmatched"}</p></div></div>)}</div> : <p className="text-[13px] text-slate-500">No line items were supplied by this channel.</p>}</div><div className="rounded-2xl border border-white/[0.06] bg-white/[0.018] p-4"><p className="text-xs font-bold uppercase tracking-[.14em] text-slate-600">Profit breakdown</p><Breakdown label="Order total" value={order.total} /><Breakdown label="Marketplace fees" value={order.marketplace_fees} negative /><Breakdown label="Shipping cost" value={order.shipping_cost} negative /><Breakdown label="Cost of goods" value={order.cost_of_goods} negative /><Breakdown label="Refunds" value={order.refund_amount} negative /><div className="mt-3 border-t border-white/[0.07] pt-3"><Breakdown label="Realized profit" value={order.net_profit} strong /></div>{order.tracking_number ? <div className="mt-4 rounded-xl bg-cyan-300/[0.045] p-3"><p className="text-xs font-bold uppercase text-cyan-300">{order.shipping_carrier || "Tracking"}</p><p className="mt-1 truncate text-[10px] text-slate-300">{order.tracking_number}</p></div> : null}</div></div></div> : null}
  </div>;
}
...
function EmptyState({ hasOrders }: { hasOrders: boolean }) { return <div className="flex min-h-[300px] flex-col items-center justify-center px-5 py-10 text-center"><div className="grid h-12 w-12 place-items-center rounded-2xl border border-cyan-300/[0.12] bg-cyan-300/[0.045]"><ShoppingBag className="h-5 w-5 text-cyan-300" /></div><h2 className="mt-4 text-base font-semibold text-white">{hasOrders ? "No orders match this view" : "No orders imported yet"}</h2><p className="mt-2 max-w-md text-[0.8rem] leading-5 text-slate-500">{hasOrders ? "Clear a filter or search for another order." : "Connect a marketplace or import channel orders. Fulfillment, item matching, costs, and realized profit will appear here."}</p>{!hasOrders ? <div className="mt-5 flex flex-wrap justify-center gap-2"><Link href="/dashboard/marketplaces" className="td-button-primary px-3.5"><Link2 className="h-3.5 w-3.5" /> Connect marketplace</Link><Link href="/dashboard/marketplaces" className="td-button-secondary px-3.5"><FileUp className="h-3.5 w-3.5" /> Import CSV</Link></div> : null}<div className="mt-6 flex flex-wrap justify-center gap-4 text-xs text-slate-500"><span className="flex items-center gap-1.5"><Check className="h-3 w-3 text-emerald-300" /> No demo orders</span><span className="flex items-center gap-1.5"><Check className="h-3 w-3 text-emerald-300" /> Duplicate-safe</span><span className="flex items-center gap-1.5"><Check className="h-3 w-3 text-emerald-300" /> Account-specific</span></div></div>; }
