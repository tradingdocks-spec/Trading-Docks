...
function TradeCenter({ binders, initialStatuses, initialRequests, initialWishlist }: { binders: PortfolioBinderView[]; initialStatuses: Array<Record<string, unknown>>; initialRequests: Array<Record<string, unknown>>; initialWishlist: Array<Record<string, unknown>> }) {
  ...
  return <div className="space-y-4">
    <section className="overflow-hidden rounded-[26px] border border-white/[0.08] bg-[linear-gradient(125deg,#07191d,#07131d_58%,#101326)] shadow-[0_22px_70px_rgba(0,0,0,.25)]"><div className="flex flex-col gap-5 p-5 lg:flex-row lg:items-center lg:justify-between"><div><div className="flex items-center gap-2"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-300/[0.09] text-emerald-300"><ArrowRightLeft className="h-4 w-4" /></span><div><p className="text-[9px] font-bold uppercase tracking-[0.17em] text-emerald-300">Trade Center</p><h2 className="text-xl font-semibold tracking-[-0.03em] text-white">Your trading command center</h2></div></div><p className="mt-3 max-w-2xl text-[11px] leading-5 text-slate-500">Choose what is available, track what you want, and manage every offer from one clean workspace.</p></div><div className="grid grid-cols-3 gap-2 sm:min-w-[380px]"><MiniMetric label="Available" value={String(available)} accent /><MiniMetric label="Wishlist" value={String(wishlist.length)} /><MiniMetric label="Active offers" value={String(activeOffers)} /></div></div><div className="flex overflow-x-auto border-t border-white/[0.07] px-3">{nav.map(({ id, label, icon: Icon, count }) => <button key={id} onClick={() => setView(id)} className={`relative flex h-12 shrink-0 items-center gap-2 px-4 text-[10px] font-semibold transition ${view === id ? "text-white after:absolute after:inset-x-4 after:bottom-0 after:h-0.5 after:bg-emerald-300" : "text-slate-600 hover:text-slate-300"}`}><Icon className="h-4 w-4" />{label}<span className={`rounded-full px-1.5 py-0.5 text-[8px] ${view === id ? "bg-emerald-300/10 text-emerald-300" : "bg-white/[0.04]"}`}>{count}</span></button>)}</div></section>
    {notice ? <p className="rounded-xl border border-amber-300/15 bg-amber-300/[0.04] px-4 py-3 text-[10px] text-amber-100">{notice}</p> : null}
    {view === "cards" ? <section className="rounded-[24px] border border-white/[0.075] bg-[#06131d]/88 p-4 sm:p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold text-white">Trade availability</p><p className="mt-1 text-[9px] text-slate-600">Set a card’s status and trade value without changing inventory.</p></div><div className="flex gap-2"><label className="flex h-10 flex-1 items-center gap-2 rounded-xl border border-white/[0.08] bg-black/20 px-3 sm:w-64"><Search className="h-3.5 w-3.5 text-slate-600" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search cards" className="min-w-0 flex-1 bg-transparent text-[10px] text-white outline-none" /></label><label className="flex h-10 items-center gap-2 rounded-xl border border-white/[0.08] bg-black/20 px-3"><SlidersHorizontal className="h-3.5 w-3.5 text-slate-600" /><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as typeof statusFilter)} className="bg-transparent text-[9px] text-slate-300 outline-none"><option value="all">All statuses</option><option value="available">Available</option><option value="reserved">Reserved</option><option value="pending">Pending</option><option value="for_sale">For sale</option><option value="looking_for_upgrade">Upgrade wanted</option><option value="not_for_trade">Not for trade</option></select></label></div></div><div className="mt-4 grid gap-2">{visibleCards.length ? visibleCards.map((card) => { const current = statusMap.get(card.id); const status = current?.status ?? "not_for_trade"; return <article key={card.id} className="grid items-center gap-3 rounded-2xl border border-white/[0.065] bg-black/10 p-2.5 sm:grid-cols-[48px_minmax(0,1fr)_145px_105px]"><div className="h-[64px] w-[46px] overflow-hidden rounded-md border border-white/10 bg-black/25">{card.imageUrl ? <img src={card.imageUrl} alt="" className="h-full w-full object-cover" /> : null}</div><div className="min-w-0"><p className="truncate text-[11px] font-semibold text-white">{card.name}</p><p className="mt-1 truncate text-[8px] uppercase tracking-[0.09em] text-slate-600">{[card.set, card.condition, card.finish, card.binderTitle].filter(Boolean).join(" · ")}</p><p className="mt-1 text-[9px] font-semibold text-emerald-300">Market {money(card.value)}</p></div><select disabled={busy === card.id} value={status} onChange={(event) => void updateCard(card.id, event.target.value as TradeStatus, current?.trade_value ?? card.value)} className="h-9 rounded-xl border border-white/[0.08] bg-[#07131d] px-2 text-[9px] text-slate-300 outline-none"><option value="not_for_trade">Not for trade</option><option value="available">Available</option><option value="reserved">Reserved</option><option value="pending">Pending</option><option value="for_sale">For sale</option><option value="looking_for_upgrade">Upgrade wanted</option></select><label className="flex h-9 items-center rounded-xl border border-white/[0.08] bg-black/20 px-2 text-[9px] text-slate-600">$<input type="number" min="0" step="0.01" defaultValue={current?.trade_value ?? card.value} onBlur={(event) => void updateCard(card.id, status, Number(event.target.value))} className="min-w-0 flex-1 bg-transparent pl-1 text-right text-white outline-none" /></label></article>; }) : <TradeEmpty icon={Tag} title="No cards match this view" body="Try another status or add cards to a binder." />}</div></section> : null}
    {view === "wishlist" ? <section className="rounded-[24px] border border-white/[0.075] bg-[#06131d]/88 p-5"><div className="flex items-center justify-between"><div><p className="text-sm font-semibold text-white">Cards you’re looking for</p><p className="mt-1 text-[9px] text-slate-600">Keep targets specific so other collectors can make better offers.</p></div><button onClick={() => setWishOpen(true)} className="inline-flex h-10 items-center gap-2 rounded-xl bg-emerald-300 px-3 text-[10px] font-bold text-[#031714]"><Plus className="h-4 w-4" /> Add card</button></div>{wishlist.length ? <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{wishlist.map((item) => <article key={item.id} className="rounded-2xl border border-white/[0.07] bg-black/15 p-4"><div className="flex items-start justify-between"><div className="min-w-0"><p className="truncate text-[12px] font-semibold text-white">{item.card_name}</p><p className="mt-1 text-[8px] uppercase tracking-[0.11em] text-slate-600">{[item.set_code, item.target_condition, item.target_finish].filter(Boolean).join(" · ") || "Any printing"}</p></div><button onClick={() => void removeWish(item.id)} className="p-2 text-slate-700 hover:text-rose-300"><Trash2 className="h-3.5 w-3.5" /></button></div><div className="mt-4 flex items-center justify-between"><span className="rounded-full bg-violet-300/[0.08] px-2 py-1 text-[8px] font-semibold capitalize text-violet-200">{item.priority}</span><span className="text-[10px] font-semibold text-emerald-300">{item.target_value ? `Up to ${money(Number(item.target_value))}` : "Open value"}</span></div></article>)}</div> : <TradeEmpty icon={Heart} title="Build your wishlist" body="Add cards you want so incoming offers are easier to evaluate." />}</section> : null}
    {view === "offers" ? <section className="rounded-[24px] border border-white/[0.075] bg-[#06131d]/88 p-5"><div><p className="text-sm font-semibold text-white">Offer pipeline</p><p className="mt-1 text-[9px] text-slate-600">Review, negotiate, accept, and complete trades without exposing addresses early.</p></div>{requests.length ? <div className="mt-4 space-y-3">{requests.map((offer) => { const giving = (offer.offered_items ?? []).reduce((sum, item) => sum + Number(item.value ?? 0), 0); const receiving = (offer.requested_items ?? []).reduce((sum, item) => sum + Number(item.value ?? 0), 0); return <article key={offer.id} className="grid gap-4 rounded-2xl border border-white/[0.07] bg-black/15 p-4 lg:grid-cols-[1fr_auto]"><div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-cyan-300/[0.08] px-2 py-1 text-[8px] font-bold uppercase tracking-[0.1em] text-cyan-200">{offer.status}</span><span className="text-[9px] text-slate-600">{offer.requester_email || "Trading Docks collector"}</span></div><div className="mt-3 flex items-center gap-3"><div><p className="text-[8px] uppercase text-slate-600">They offer</p><p className="mt-1 text-sm font-semibold text-white">{money(giving)}</p></div><ArrowRightLeft className="h-4 w-4 text-slate-700" /><div><p className="text-[8px] uppercase text-slate-600">They request</p><p className="mt-1 text-sm font-semibold text-white">{money(receiving)}</p></div><span className={`ml-1 text-[9px] font-semibold ${Math.abs(giving - receiving) < 2 ? "text-emerald-300" : "text-amber-300"}`}>{giving === receiving ? "Even" : `${money(Math.abs(giving - receiving))} difference`}</span></div>{offer.message ? <p className="mt-3 text-[9px] leading-5 text-slate-500">{offer.message}</p> : null}</div><div className="flex items-center gap-2">{offer.status === "new" ? <><button disabled={busy === offer.id} onClick={() => void updateOffer(offer.id, "reviewing")} className="h-9 rounded-xl border border-white/[0.09] px-3 text-[9px] font-semibold text-white">Review</button><button disabled={busy === offer.id} onClick={() => void updateOffer(offer.id, "accepted")} className="h-9 rounded-xl bg-emerald-300 px-3 text-[9px] font-bold text-[#031714]">Accept</button></> : offer.status === "accepted" ? <button disabled={busy === offer.id} onClick={() => void updateOffer(offer.id, "completed")} className="inline-flex h-9 items-center gap-2 rounded-xl bg-cyan-300 px-3 text-[9px] font-bold text-[#031319]"><PackageCheck className="h-3.5 w-3.5" /> Complete</button> : <button className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/[0.08] text-slate-600"><MoreHorizontal className="h-4 w-4" /></button>}</div></article>; })}</div> : <TradeEmpty icon={Send} title="No offers yet" body="Mark cards available and publish a trade binder to start receiving offers." />}</section> : null}
    {wishOpen ? <WishlistModal onClose={() => setWishOpen(false)} onSaved={(item) => { setWishlist((current) => [item, ...current]); setWishOpen(false); }} /> : null}
  </div>;
}

function TradeEmpty({ icon: Icon, title, body }: { icon: typeof Tag; title: string; body: string }) { return <div className="mt-4 rounded-2xl border border-dashed border-white/[0.09] py-12 text-center"><Icon className="mx-auto h-6 w-6 text-slate-700" /><p className="mt-3 text-sm font-semibold text-slate-300">{title}</p><p className="mt-1 text-[9px] text-slate-600">{body}</p></div>; }
function WishlistModal({ onClose, onSaved }: { onClose: () => void; onSaved: (item: WishlistRow) => void }) { const [name, setName] = useState(""); const [setCode, setSetCode] = useState(""); const [condition, setCondition] = useState("Near Mint"); const [value, setValue] = useState(""); const [priority, setPriority] = useState("medium"); const [busy, setBusy] = useState(false); async function save() { if (!name.trim()) return; setBusy(true); const response = await fetch("/api/collector-portfolio/wishlist", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ cardName: name, setCode, targetCondition: condition, targetValue: value ? Number(value) : null, priority }) }); const result = await response.json().catch(() => null) as { item?: WishlistRow } | null; if (response.ok && result?.item) onSaved(result.item); setBusy(false); } return <div className="fixed inset-0 z-[120] flex items-center justify-center bg-[#01070c]/85 p-4 backdrop-blur-xl" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="w-full max-w-[520px] rounded-[28px] border border-emerald-300/[0.16] bg-[#07131d] p-6 shadow-[0_40px_130px_rgba(0,0,0,.7)]"><div className="flex items-start justify-between"><div><p className="text-[9px] font-bold uppercase tracking-[0.16em] text-emerald-300">Wishlist</p><h3 className="mt-2 text-xl font-semibold text-white">Add a card you want</h3></div><button onClick={onClose} className="p-2 text-slate-600 hover:text-white"><X className="h-4 w-4" /></button></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><Field label="Card name" wide><input autoFocus value={name} onChange={(event) => setName(event.target.value)} className="portfolio-input" placeholder="e.g. Rhystic Study" /></Field><Field label="Set code"><input value={setCode} onChange={(event) => setSetCode(event.target.value.toUpperCase())} className="portfolio-input" placeholder="Any" /></Field><Field label="Condition"><select value={condition} onChange={(event) => setCondition(event.target.value)} className="portfolio-input"><option>Near Mint</option><option>Lightly Played</option><option>Any</option></select></Field><Field label="Maximum value"><input type="number" min="0" value={value} onChange={(event) => setValue(event.target.value)} className="portfolio-input" placeholder="$0.00" /></Field><Field label="Priority"><select value={priority} onChange={(event) => setPriority(event.target.value)} className="portfolio-input"><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="grail">Grail</option></select></Field></div><button disabled={busy || !name.trim()} onClick={() => void save()} className="mt-6 inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-emerald-300 text-[10px] font-bold text-[#031714] disabled:opacity-50">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Heart className="h-4 w-4" />} Add to wishlist</button></section></div>; }
