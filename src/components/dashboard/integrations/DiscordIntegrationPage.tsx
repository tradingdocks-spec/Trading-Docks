...
export function DiscordIntegrationPage({
  ...
  async function refreshChannels() {
    ...
    try {
      ...
      setSelectedChannel((current) => nextChannels.some((channel) => channel.id === current && channel.enabled && channel.can_send)
        ? current
        : nextChannels.find((channel) => channel.enabled && channel.can_send)?.id ?? "");
  ...
  async function sendMessage() {
    ...
    if (!selectedChannel || !channel || !channel.enabled || !channel.can_send) {
      setFeedback("Choose an authorized channel before sending.");
      return;
    }
