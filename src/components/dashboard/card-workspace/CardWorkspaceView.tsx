    ...
    <main className="min-h-screen bg-td-canvas px-4 py-4 text-td-primary sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[1500px] space-y-4">
        ...
        <section className="grid gap-4 xl:grid-cols-[minmax(260px,360px)_minmax(0,1fr)]">
          <div className="overflow-hidden rounded-[28px] bg-td-surface shadow-[0_26px_110px_rgb(var(--td-shadow-rgb)/calc(.42*var(--td-shadow-strength)))] ring-1 ring-td-ink/[0.07]">
            {data.identity.imageUrl ? (
              ...
              <div className="flex aspect-[0.716] flex-col items-center justify-center gap-3 bg-td-ink/[0.025] text-td-muted">
          ...
          <div className="rounded-[30px] bg-td-surface p-5 shadow-[0_22px_90px_rgb(var(--td-shadow-rgb)/calc(.28*var(--td-shadow-strength)))] ring-1 ring-td-ink/[0.07] sm:p-6">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
              <div className="min-w-0">
                <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-td-accent-text/80">{data.printing.display}</p>
                <h1 className="mt-2 max-w-4xl text-4xl font-semibold tracking-[-0.055em] text-td-primary sm:text-6xl">{data.identity.cardName}</h1>
              ...
              <div className="rounded-2xl bg-td-accent/[0.08] px-4 py-3 text-left ring-1 ring-td-accent/[0.16] lg:text-right">
                <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-td-accent-text/75">Current price</p>
                <p className="mt-1 text-3xl font-semibold tracking-[-0.045em] text-td-accent-text">{data.market.valueLabel}</p>
                <p className="mt-1 text-xs text-td-accent-text/55">{data.market.valueSource === "inventory" ? "Saved inventory valuation" : "Valuation unavailable"}</p>
              </div>
            </div>
                ...
                {data.inventoryRecords.map((record) => (
                  <article key={record.id} className="rounded-2xl bg-td-ink/[0.035] p-4 ring-1 ring-td-ink/[0.06] transition hover:bg-td-ink/[0.05]">
                    <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_100px_130px_120px] lg:items-center">
                      ...
                      <Link href={record.editHref} className="inline-flex h-9 items-center justify-center rounded-xl border border-td-ink/[0.08] px-3 text-xs font-bold text-td-secondary transition hover:border-td-accent/30 hover:text-td-accent-text">Edit</Link>
                    </div>
                    {record.attentionTypes.length ? (
                      <div className="mt-3 flex flex-wrap gap-2">
                        {record.attentionTypes.map((type) => (
                          <span key={type} className="inline-flex items-center gap-1.5 rounded-xl bg-td-warning/[0.08] px-2.5 py-1.5 text-[11px] font-semibold text-td-warning">
              ...
              {data.otherPrintings.length ? (
                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                  {data.otherPrintings.map((printing) => (
                    <a key={printing.id} href={printing.href ?? "#"} target={printing.href ? "_blank" : undefined} rel="noreferrer" className="group overflow-hidden rounded-2xl bg-td-ink/[0.035] ring-1 ring-td-ink/[0.06] transition hover:ring-td-accent/20">
                      {printing.imageUrl ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={printing.imageUrl} alt={`${printing.name} printing`} className="aspect-[0.716] w-full object-cover" />
                      ) : (
                        <div className="flex aspect-[0.716] items-center justify-center text-td-muted"><ImageIcon className="h-7 w-7" /></div>
                      )}
                      <div className="p-3">
                        <p className="truncate text-xs font-semibold text-td-primary">{printing.setCode ?? "Set"} {printing.collectorNumber ? `#${printing.collectorNumber}` : ""}</p>
                        <p className="mt-1 text-xs text-td-muted">{money(printing.price)}</p>
                      </div>
                    </a>
                  ))}
                </div>
          ...
          <aside className="space-y-4">
            ...
            <Panel title="Deck Context" eyebrow={data.decks.length ? `Used in ${data.decks.length}` : "No deck usage found"}>
              {data.decks.length ? data.decks.map((deck) => (
                <Link key={deck.id} href={deck.href} className="flex items-center justify-between gap-3 border-b border-td-ink/[0.06] py-2 text-sm last:border-b-0">
                  <span className="truncate font-semibold text-td-primary">{deck.name}</span>
                  <span className="text-xs text-td-muted">x{deck.quantity}</span>
                </Link>
...
function Panel({ title, eyebrow, children }: { title: string; eyebrow: string; children: ReactNode }) {
  return (
    <section className="rounded-[26px] bg-td-surface p-5 ring-1 ring-td-ink/[0.07]">
      <div className="mb-4 flex items-end justify-between gap-3">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-td-muted">{eyebrow}</p>
          <h2 className="mt-1 text-xl font-semibold tracking-[-0.035em] text-td-primary">{title}</h2>
        </div>
        <Layers3 className="h-4 w-4 text-td-muted" />
      </div>
      {children}
    </section>
  );
...
function Metric({ label, value, detail }: { label: string; value: string; detail: string }) {
  return (
    <div className="rounded-2xl bg-td-ink/[0.035] p-4 ring-1 ring-td-ink/[0.06]">
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-3 border-b border-td-ink/[0.06] py-2 text-sm last:border-b-0">
      <span className="text-td-muted">{label}</span>
      <span className="max-w-[210px] break-words text-right font-semibold text-td-primary">{value}</span>
    </div>
  );
}

function EmptyLine({ text }: { text: string }) {
  return <p className="rounded-2xl bg-td-ink/[0.025] p-4 text-xs leading-5 text-td-muted">{text}</p>;
}

function actionClass(priority: "primary" | "secondary" | "tertiary") {
  const base = "inline-flex min-h-10 items-center justify-center gap-2 rounded-xl px-3.5 text-xs font-bold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-td-accent/60";
  if (priority === "primary") return `${base} bg-td-accent text-td-on-accent hover:bg-td-accent-hover`;
  if (priority === "secondary") return `${base} border border-td-ink/[0.08] text-td-primary hover:border-td-accent/30 hover:text-td-accent-text`;
  return `${base} text-td-muted hover:text-td-primary`;
}
