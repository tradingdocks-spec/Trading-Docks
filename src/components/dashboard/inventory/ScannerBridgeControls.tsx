...
export function ScannerBridgeControls({ disabled, active, onReady, onUnavailable }: { disabled: boolean; active: boolean; onReady: (provider: TradingDocksLocalScannerProvider) => void; onUnavailable: () => void }) {
  ...
  return <div className="rounded-xl border p-3 space-y-3" aria-label="Scanner Bridge connection">
    ...
    {device && settings && <>
      ...
      {device.scanCapabilities?.setupRequired && <p role="status">Finish ScanSnap setup in the bridge tray, confirm the profile, then refresh devices.</p>}{device.scanCapabilities?.captureInstruction && <p role="note" className="text-sm">{device.scanCapabilities.captureInstruction}</p>}
      ...
      {showSettings && device.scanCapabilities?.externalSettings && <p className="text-sm">Configure the required profile in the scanner application before capture. The bridge cannot apply or verify these driver settings. Configure automatic JPEG direct save to the permanent Inbox once using ScanSnap setup in the bridge tray. No per-card filenames or copying. Physical acceptance is pending.</p>}
