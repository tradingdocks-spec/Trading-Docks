...
export function ScannerBridgeControls({ disabled, active, onReady, onUnavailable }: { disabled: boolean; active: boolean; onReady: (provider: TradingDocksLocalScannerProvider) => void; onUnavailable: () => void }) {
  ...
  return <div className="rounded-xl border p-3 space-y-3" aria-label="Scanner Bridge connection">
    ...
    <div className="flex flex-wrap gap-2">
      ...
      {found && !paired && <TDButton size="sm" disabled={disabled || busy} onClick={() => void pair()}>Pair this workstation</TDButton>}
    ...
    {paired && <label className="block">Scanner<select aria-label="Installed scanner" className="block w-full border rounded p-2" value={selected} disabled={disabled || busy} onChange={async event => { const next = devices.find(d => d.id === event.target.value); if (!next) return; setBusy(true); try { await choose(next); } catch (error) { setStatus(error instanceof Error ? error.message : "Scanner disconnected"); callbacks.current.onUnavailable(); } finally { setBusy(false); } }}><option value="">Select a scanner</option>{devices.map(d => <option key={d.id} value={d.id}>{d.name} · {d.connection} · Detected / untested</option>)}</select></label>}
    {device && settings && <>
      <label className="block">Profile<select aria-label="Scanner profile" className="block border rounded p-2" disabled={disabled || busy} value={profile} onChange={event => configure(defaultScanSettings(device.scanCapabilities!, event.target.value === "quality"), event.target.value)}><option value="fast">Trading Cards — Fast</option><option value="quality">Trading Cards — High Quality</option>{profile === "custom" && <option value="custom">Custom scanner settings</option>}</select></label>
      <p className="text-sm">{settings.dpi} DPI · {settings.colorMode} · {settings.source} · Single-sided{settings.autoCrop ? " · Auto crop" : ""}</p>
      <p className="text-xs">V1 captures a 3 × 4 inch region. Align one card at the scanner origin. Automatic cropping and duplex are available only when reported by the bridge.</p>
      <TDButton size="sm" variant="secondary" onClick={() => setShowSettings(v => !v)}>Scanner Settings</TDButton>
      {showSettings && <fieldset disabled={disabled || busy} className="flex flex-wrap gap-3"><label>DPI<select aria-label="Scanner DPI" value={settings.dpi} onChange={e => configure({ ...settings, dpi: Number(e.target.value) })}>{device.scanCapabilities!.dpi.map(d => <option key={d}>{d}</option>)}</select></label><label>Source<select aria-label="Scanner source" value={settings.source} onChange={e => configure({ ...settings, source: e.target.value })}>{device.scanCapabilities!.sources.map(s => <option key={s}>{s}</option>)}</select></label><label>Color<select aria-label="Scanner color" value={settings.colorMode} onChange={e => configure({ ...settings, colorMode: e.target.value })}>{device.scanCapabilities!.colorModes.map(c => <option key={c}>{c}</option>)}</select></label>{device.scanCapabilities!.autoCrop && <label><input type="checkbox" checked={settings.autoCrop} onChange={e => configure({ ...settings, autoCrop: e.target.checked })} />Auto crop</label>}{device.scanCapabilities!.duplex && <label><input type="checkbox" checked={settings.duplex} onChange={e => configure({ ...settings, duplex: e.target.checked })} />Duplex</label>}</fieldset>}
    </>}
  </div>;
}
