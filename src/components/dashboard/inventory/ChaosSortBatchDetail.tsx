  ...
  return <main className="batch-page min-h-screen bg-[var(--td-background-primary)] p-4 text-white sm:p-6 lg:p-8">
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <section className="grid gap-5 lg:grid-cols-[1fr_240px]">
        ...
        <aside className="label-sheet flex h-[90mm] w-[29mm] flex-col items-center justify-start rounded-2xl border border-white/10 bg-white px-[3mm] pb-[3mm] pt-[2mm] text-center text-slate-950"><p className="text-[20px] font-black leading-none tracking-tight">{data.batch.batch_code}</p>{qr ? <img src={qr} alt={`QR code for ${data.batch.batch_code}`} className="mx-auto mt-[4mm] h-[22mm] w-[22mm]" /> : <div className="mx-auto mt-[4mm] h-[22mm] w-[22mm] animate-pulse bg-slate-200" />}<p className="mt-[4mm] text-[11px] font-bold leading-tight">{data.batch.current_quantity} / {data.batch.initial_quantity} cards</p><p className="mt-[2mm] text-[10px] uppercase tracking-wide">{new Date(data.batch.created_at).toLocaleDateString()}</p></aside>
      </section>
    ...
    <style>{`@page { size: 29mm 90mm; margin: 0; } @media print { body * { visibility: hidden !important; } .label-sheet, .label-sheet * { visibility: visible !important; } .label-sheet { position: absolute; left: 0; top: 0; width: 29mm; height: 90mm; border: 0; border-radius: 0; } }`}</style>
  </main>;
}
