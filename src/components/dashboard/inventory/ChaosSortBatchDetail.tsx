...
export function ChaosSortBatchDetail({ data }: { data: ChaosSortBatchDetailData }) {
  ...
  useEffect(() => {
    ...
    if (!saved) return;
    try {
      const parsed = JSON.parse(saved) as { key?: LabelMediaKey; width?: string; height?: string };
  ...
  function updateMedia(key: LabelMediaKey) {
    setMediaKey(key);
    window.localStorage.setItem("td.batch-label-media", JSON.stringify({ key, width: customWidth, height: customHeight }));
  }
  function updateCustomMedia(width: string, height: string) {
    setCustomWidth(width); setCustomHeight(height);
    window.localStorage.setItem("td.batch-label-media", JSON.stringify({ key: "custom", width, height }));
  }
  ...
  return <main className="batch-page min-h-screen bg-[var(--td-background-primary)] p-4 text-white sm:p-6 lg:p-8">
    ...
    <style>{`@media print { @page { size: var(--label-width) var(--label-height); margin: 0; } body * { visibility: hidden !important; } .label-sheet, .label-sheet * { visibility: visible !important; } .label-sheet { position: absolute; left: 0; top: 0; width: var(--label-width); height: var(--label-height); min-height: 0; overflow: hidden; border: 0; border-radius: 0; padding: 3mm; } }`}</style>
  </main>;
}
