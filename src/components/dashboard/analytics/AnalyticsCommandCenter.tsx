    ...
    <WorkspaceFrame>
      <div className="space-y-4 sm:space-y-5">
          ...
          <div className="relative px-4 py-5 sm:px-6 sm:py-6 lg:px-7">
            ...
            <div className="relative flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
              <div>
                ...
                <h1 className="mt-4 text-3xl font-semibold tracking-[-0.05em] text-white sm:text-4xl">Know what is working.</h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">Track profitability, inventory velocity, channel performance, and the next actions that deserve your attention.</p>
              </div>
              <button type="button" onClick={exportReport} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-white/[0.1] bg-white/[0.045] px-4 text-xs font-semibold text-slate-200 transition hover:border-cyan-300/25 hover:bg-cyan-300/[0.07]">
        ...
        <section className="grid gap-4 xl:grid-cols-[minmax(0,1.65fr)_minmax(330px,.75fr)]">
          <Panel className="min-h-[390px]" title="Revenue & profitability" eyebrow="Performance over time" icon={Activity} action={
            ...
            <div className="relative mt-5 h-[245px] overflow-hidden rounded-2xl border border-white/[0.055] bg-[linear-gradient(180deg,rgba(34,211,238,.025),transparent)]">
              <div className="absolute inset-0 grid grid-rows-4">
                {[0, 1, 2, 3].map((line) => <div key={line} className="border-b border-dashed border-white/[0.055]" />)}
              </div>
              <div className="absolute inset-0 flex flex-col items-center justify-center px-5 text-center">
                <div className="grid h-12 w-12 place-items-center rounded-2xl border border-cyan-300/[0.14] bg-cyan-300/[0.055]"><TrendingUp className="h-5 w-5 text-cyan-300" /></div>
                <p className="mt-4 text-sm font-semibold text-white">Your {metric.toLowerCase()} trend will appear here</p>
                <p className="mt-1 max-w-sm text-[11px] leading-5 text-slate-500">Import marketplace orders or record sales to unlock daily, weekly, and monthly performance.</p>
                <Link href="/dashboard/marketplaces" className="mt-4 inline-flex items-center gap-2 text-[10px] font-bold text-cyan-300 hover:text-cyan-200">Connect sales data <ArrowRight className="h-3.5 w-3.5" /></Link>
              </div>
            </div>
...
function Panel({ title, eyebrow, icon: Icon, action, badge, className = "", children }: { title: string; eyebrow: string; icon: LucideIcon; action?: React.ReactNode; badge?: string; className?: string; children: React.ReactNode }) {
  return <article className={`rounded-[24px] border border-white/[0.075] bg-[rgba(6,18,27,.83)] p-4 shadow-[0_18px_55px_rgba(0,0,0,.16)] backdrop-blur-xl sm:p-5 ${className}`}><div className="flex flex-wrap items-start justify-between gap-3"><div className="flex items-start gap-3"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-cyan-300/[0.11] bg-cyan-300/[0.045]"><Icon className="h-4 w-4 text-cyan-300" /></div><div><p className="text-[8px] font-bold uppercase tracking-[0.16em] text-slate-600">{eyebrow}</p><h2 className="mt-1 text-sm font-semibold tracking-[-0.02em] text-white">{title}</h2></div></div>{badge ? <span className="rounded-full border border-emerald-300/[0.13] bg-emerald-300/[0.045] px-2.5 py-1 text-[8px] font-bold uppercase tracking-wider text-emerald-300">{badge}</span> : action}</div>{children}</article>;
}
...
function MetricCard({ label, value, detail, icon: Icon, highlight }: { label: string; value: string; detail: string; icon: LucideIcon; highlight?: boolean }) {
  return <article className={`group relative overflow-hidden rounded-[21px] border p-4 transition hover:-translate-y-0.5 ${highlight ? "border-cyan-300/[0.17] bg-cyan-300/[0.045]" : "border-white/[0.07] bg-[rgba(6,18,27,.8)]"}`}><div className="flex items-start justify-between gap-3"><span className="text-[9px] font-bold uppercase tracking-[0.13em] text-slate-600">{label}</span><Icon className={`h-4 w-4 ${highlight ? "text-cyan-300" : "text-slate-600"}`} /></div><p className="mt-5 text-2xl font-semibold tracking-[-0.04em] text-white">{value}</p><p className="mt-1 truncate text-[9px] text-slate-600" title={detail}>{detail}</p><div className="absolute inset-x-0 bottom-0 h-px bg-gradient-to-r from-transparent via-cyan-300/0 to-transparent transition group-hover:via-cyan-300/30" /></article>;
}
