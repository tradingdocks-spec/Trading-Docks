...
import {
  BadgeDollarSign, Building2, Check, ChevronDown, ChevronRight, ContactRound, Gift, History, Import,
  ...
  return <WorkspaceFrame>
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.65fr)_minmax(330px,.85fr)]">
      ...
      <aside className="space-y-5">
        ...
        <section className={`${styles.glassPanel} rounded-[26px] p-5`}><div className="flex items-center gap-3"><History className="h-5 w-5 text-violet-300" /><div><h2 className="text-sm font-semibold text-white">Recent activity</h2><p className="mt-1 text-[11px] text-slate-500">Latest customer records in this workspace</p></div></div><div className="mt-4 space-y-2">{customers.slice(0,3).map((customer) => <button key={customer.id} type="button" onClick={() => { setSelected(customer); void loadLedger(customer.id); }} className="flex w-full items-center gap-3 rounded-xl border border-white/[0.06] bg-white/[0.018] p-3 text-left transition hover:border-cyan-300/[0.14]"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-violet-300/[0.07] text-[9px] font-bold text-violet-200">{initials(customer)}</span><span className="min-w-0 flex-1"><span className="block truncate text-[11px] font-semibold text-slate-200">{customer.first_name} {customer.last_name}</span><span className="mt-0.5 block text-[9px] text-slate-600">Profile added {new Date(customer.created_at).toLocaleDateString()}</span></span><ChevronRight className="h-3.5 w-3.5 text-slate-700" /></button>)}{!customers.length ? <p className="rounded-xl border border-dashed border-white/[0.07] p-4 text-center text-[10px] text-slate-600">Customer activity will appear here.</p> : null}</div></section>
      </aside>
    </div>
...
function TextField({ label, value, onChange, type = "text", required = false, placeholder }: { label: string; value: string; onChange: (value: string) => void; type?: string; required?: boolean; placeholder?: string }) { return <label><span className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">{label}</span><input required={required} type={type} value={value} placeholder={placeholder} onChange={(e) => onChange(e.target.value)} className="workspace-input mt-2 w-full placeholder:text-slate-700" /></label>; }
...
function SelectField({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: readonly (readonly [string,string])[] }) { return <label><span className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">{label}</span><select value={value} onChange={(e) => onChange(e.target.value)} className="workspace-input mt-2 w-full appearance-none">{options.map(([optionValue, optionLabel]) => <option key={optionValue} value={optionValue}>{optionLabel}</option>)}</select></label>; }
function FormSection({ icon: Icon, title, description, children }: { icon: typeof UserRound; title: string; description: string; children: React.ReactNode }) { return <section className="mb-4 rounded-2xl border border-white/[0.07] bg-white/[0.018] p-4"><div className="mb-4 flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl border border-cyan-300/[0.12] bg-cyan-300/[0.05] text-cyan-200"><Icon className="h-4 w-4" /></span><div><h3 className="text-xs font-semibold text-white">{title}</h3><p className="mt-0.5 text-[10px] text-slate-600">{description}</p></div></div>{children}</section>; }
