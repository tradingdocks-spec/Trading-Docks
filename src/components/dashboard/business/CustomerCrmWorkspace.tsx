...
export function CustomerCrmWorkspace() {
  ...
  return <WorkspaceFrame>
    ...
    {savedNotice ? <div className="fixed right-4 top-4 z-[70] flex items-center gap-2 rounded-2xl border border-td-success/20 bg-td-surface/95 px-4 py-3 text-xs font-semibold text-td-success shadow-2xl backdrop-blur-xl"><Check className="h-4 w-4 text-td-success" />{savedNotice}</div> : null}
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.65fr)_minmax(330px,.85fr)]">
      <section className={`${styles.glassPanel} overflow-hidden rounded-[26px]`}>
        <div className="border-b border-td-ink/[0.06] p-4 sm:p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-td-accent-text">Customer directory</p><h2 className="mt-1.5 text-lg font-semibold text-td-primary">Relationship hub</h2></div>
          <label className="flex h-10 items-center gap-2 rounded-xl border border-td-ink/[0.08] bg-black/20 px-3 transition focus-within:border-td-accent/30 focus-within:ring-2 focus-within:ring-td-accent/[0.06] sm:w-72"><Search className="h-4 w-4 text-td-muted" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search customers, tags…" className="min-w-0 flex-1 bg-transparent text-xs text-td-primary outline-none placeholder:text-td-muted" /></label></div>
          <div className="mt-4 flex gap-2 overflow-x-auto pb-1">{([['all','All customers'],['marketing','Marketing eligible'],['credit','Has store credit'],['loyalty','Loyalty members']] as const).map(([value,label]) => <button key={value} type="button" onClick={() => setDirectoryFilter(value)} className={`whitespace-nowrap rounded-full border px-3 py-1.5 text-[11px] font-semibold transition ${directoryFilter === value ? "border-td-accent/25 bg-td-accent/[0.1] text-td-accent-text shadow-[0_0_18px_rgb(var(--td-accent-rgb)/.08)]" : "border-td-ink/[0.07] bg-td-ink/[0.02] text-td-muted hover:border-td-ink/[0.13] hover:text-td-secondary"}`}>{label}</button>)}</div>
        </div>
        {loading ? <Loader2 className="mx-auto my-16 h-6 w-6 animate-spin text-td-accent-text" /> : filtered.length ? <div className="divide-y divide-td-ink/[0.055]">{filtered.map((customer) => <button key={customer.id} type="button" onClick={() => { setSelected(customer); void loadLedger(customer.id); }} className="group flex w-full items-center gap-3 p-4 text-left transition hover:bg-td-accent/[0.035] sm:px-5">
          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-td-accent/[0.14] bg-td-accent/[0.055] text-xs font-bold text-td-accent-text">{initials(customer)}</span>
          <span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-td-primary">{customer.first_name} {customer.last_name}</span><span className="mt-1 block truncate text-[11px] text-td-muted">{customer.email || customer.phone || "No contact details"}</span><span className="mt-2 flex flex-wrap gap-1">{customer.tags.slice(0, 3).map((tag) => <span key={tag} className="rounded-full border border-td-accent/[0.1] bg-td-accent/[0.035] px-2 py-0.5 text-[11px] font-semibold text-td-accent-text/70">{tag}</span>)}</span></span>
          <span className="hidden text-right sm:block"><span className="block text-xs font-semibold text-td-success">{money(customer.store_credit_cents)} credit</span><span className="mt-1 block text-[11px] text-td-warning/70">{customer.loyalty_points.toLocaleString()} pts</span></span><ChevronRight className="h-4 w-4 text-td-muted transition group-hover:translate-x-0.5 group-hover:text-td-accent-text" />
        </button>)}</div> : <div className="m-5 flex min-h-64 flex-col items-center justify-center rounded-[22px] border border-dashed border-td-accent/[0.14] bg-gradient-to-b from-td-accent/[0.035] to-transparent px-5 text-center"><span className="flex h-12 w-12 items-center justify-center rounded-2xl border border-td-accent/[0.15] bg-td-accent/[0.07] shadow-[0_0_30px_rgb(var(--td-accent-rgb)/.08)]"><UserRound className="h-5 w-5 text-td-accent-text" /></span><span className="mt-4 text-base font-semibold text-td-primary">{customers.length ? "No customers match this view" : "Build your customer network"}</span><span className="mt-1.5 max-w-md text-xs leading-5 text-td-muted">{customers.length ? "Try another filter or search term." : "Create a profile to unlock auditable store credit, loyalty rewards, customer notes, and consent-based marketing."}</span><div className="mt-5 flex flex-col items-center gap-3"><button type="button" onClick={openNewCustomer} className="inline-flex h-10 items-center gap-2 rounded-xl bg-td-accent px-4 text-xs font-bold text-td-on-accent shadow-[0_10px_25px_rgb(var(--td-accent-rgb)/.18)]"><Plus className="h-4 w-4" />Add first customer</button><p className="max-w-sm text-[11px] leading-5 text-td-muted">Bulk customer import is intentionally unavailable until the consent and duplicate-review workflow is connected.</p></div></div>}
      </section>

      <aside className="space-y-5">
        <section className={`${styles.glassPanel} rounded-[22px] p-5`}><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-td-warning/[0.08] text-td-warning"><Gift className="h-4 w-4" /></span><div><p className="text-[0.68rem] font-bold uppercase tracking-[0.14em] text-td-warning/70">Loyalty engine</p><h2 className="mt-1 text-base font-semibold text-td-primary">Program rules</h2></div><button type="button" aria-label={loyalty.enabled ? "Pause loyalty program" : "Activate loyalty program"} onClick={() => setLoyalty({ ...loyalty, enabled: !loyalty.enabled })} className={`ml-auto rounded-full px-3 py-1.5 text-[0.68rem] font-bold uppercase tracking-wider ${loyalty.enabled ? "bg-td-success/10 text-td-success" : "bg-td-ink/[0.04] text-td-muted"}`}>{loyalty.enabled ? "Active" : "Paused"}</button></div>
          ...
          <div className="mt-4 rounded-xl border border-td-accent/[0.09] bg-td-accent/[0.025] p-3 text-[11px] leading-5 text-td-muted"><Sparkles className="mr-2 inline h-3.5 w-3.5 text-td-accent-text" />Customers earn {loyalty.points_per_dollar} point{loyalty.points_per_dollar === 1 ? "" : "s"} per dollar and unlock {money(loyalty.reward_value_cents)} at {loyalty.reward_points} points.</div>
          <div className="mt-3 flex items-center justify-between rounded-xl border border-td-warning/[0.12] bg-td-warning/[0.035] px-3 py-2.5"><span className="text-[11px] text-td-muted">Example at current settings</span><span className="text-[11px] font-semibold text-td-warning">Spend {money(Math.ceil(loyalty.reward_points / Math.max(loyalty.points_per_dollar, .01)) * 100)} → earn {money(loyalty.reward_value_cents)}</span></div>
          <button type="button" disabled={saving} onClick={() => void saveLoyalty()} className="mt-4 h-10 w-full rounded-xl bg-td-accent text-xs font-bold text-td-on-accent disabled:opacity-50">Save loyalty rules</button>
        </section>
        <section className={`${styles.glassPanel} rounded-[22px] p-5`}><div className="flex items-center gap-3"><ShieldCheck className="h-5 w-5 text-td-success" /><div><h2 className="text-sm font-semibold text-td-primary">Consent-first marketing</h2><p className="mt-1 text-[0.8rem] leading-5 text-td-muted">Audience eligibility is based on each customer’s recorded email and SMS consent.</p></div></div><div className="mt-4 grid grid-cols-2 gap-2"><AudienceStat label="Email" value={customers.filter((c) => c.marketing_email_consent).length} /><AudienceStat label="SMS" value={customers.filter((c) => c.marketing_sms_consent).length} /></div></section>
        <section className={`${styles.glassPanel} rounded-[22px] p-5`}><div className="flex items-center gap-3"><History className="h-5 w-5 text-td-violet" /><div><h2 className="text-sm font-semibold text-td-primary">Recent activity</h2><p className="mt-1 text-[0.8rem] text-td-muted">Latest customer records in this workspace</p></div></div><div className="mt-4 space-y-2">{customers.slice(0,3).map((customer) => <button key={customer.id} type="button" onClick={() => { setSelected(customer); void loadLedger(customer.id); }} className="flex w-full items-center gap-3 rounded-xl border border-td-ink/[0.06] bg-td-ink/[0.018] p-3 text-left transition hover:border-td-accent/[0.14]"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-td-violet/[0.07] text-[0.68rem] font-bold text-td-violet">{initials(customer)}</span><span className="min-w-0 flex-1"><span className="block truncate text-xs font-semibold text-td-primary">{customer.first_name} {customer.last_name}</span><span className="mt-0.5 block text-[0.72rem] text-td-muted">Profile added {new Date(customer.created_at).toLocaleDateString()}</span></span><ChevronRight className="h-3.5 w-3.5 text-td-muted" /></button>)}{!customers.length ? <p className="rounded-xl border border-dashed border-td-ink/[0.07] p-4 text-center text-xs text-td-muted">Customer activity will appear here.</p> : null}</div></section>
      </aside>
    </div>

    {editorOpen ? <Modal onClose={() => setEditorOpen(false)}><form onSubmit={saveCustomer} className="flex max-h-[calc(100dvh-2rem)] flex-col">
      <div className="shrink-0 border-b border-td-ink/[0.07] px-5 py-4 sm:px-6"><ModalHeader title={editingId ? "Edit customer profile" : "Create customer profile"} subtitle="Build one reliable record for every customer interaction." onClose={() => setEditorOpen(false)} /></div>
      <div className="min-h-0 overflow-y-auto px-5 py-5 sm:px-6">
        ...
        <FormSection icon={Building2} title="Relationship details" description="Segment this profile for reporting and outreach.">
          ...
          <div className="mt-3"><p className="text-[11px] font-semibold uppercase tracking-wider text-td-secondary">Customer tags</p><div className="mt-2 flex min-h-11 flex-wrap items-center gap-2 rounded-xl border border-td-ink/[0.08] bg-black/20 px-3 py-2 focus-within:border-td-accent/30">{form.tags.map((tag) => <span key={tag} className="inline-flex items-center gap-1 rounded-full border border-td-accent/[0.16] bg-td-accent/[0.07] px-2.5 py-1 text-[11px] font-semibold text-td-accent-text">{tag}<button type="button" aria-label={`Remove ${tag}`} onClick={() => setForm({ ...form, tags: form.tags.filter((item) => item !== tag) })}><X className="h-3 w-3" /></button></span>)}<input value={form.tagDraft} onChange={(e) => setForm({ ...form, tagDraft: e.target.value })} onKeyDown={(e) => { if ((e.key === "Enter" || e.key === ",") && form.tagDraft.trim()) { e.preventDefault(); const next = form.tagDraft.trim().replace(/,$/, ""); if (!form.tags.includes(next)) setForm({ ...form, tags: [...form.tags, next], tagDraft: "" }); } }} placeholder={form.tags.length ? "Add another…" : "VIP, Commander player, Local…"} className="min-w-36 flex-1 bg-transparent text-xs text-td-primary outline-none placeholder:text-td-muted" /></div><p className="mt-1.5 text-[11px] text-td-muted">Press Enter to add each tag.</p></div>
        </FormSection>
        <FormSection icon={ShieldCheck} title="Marketing consent" description="Only record permission the customer explicitly provided.">
          <div className="grid gap-2 sm:grid-cols-2"><ConsentToggle label="Email marketing" detail="Offers and store updates" checked={form.marketing_email_consent} onChange={(checked) => setForm({ ...form, marketing_email_consent: checked })} /><ConsentToggle label="SMS marketing" detail="Texts and time-sensitive alerts" checked={form.marketing_sms_consent} onChange={(checked) => setForm({ ...form, marketing_sms_consent: checked })} /></div><p className="mt-3 flex items-center gap-2 text-[11px] text-td-muted"><ShieldCheck className="h-3 w-3 text-td-success" />Consent date and staff source are recorded when this profile is saved.</p>
        </FormSection>
        <div className="rounded-2xl border border-td-ink/[0.07] bg-td-ink/[0.018]"><button type="button" onClick={() => setNotesOpen(!notesOpen)} className="flex w-full items-center gap-3 p-4 text-left"><MessageSquareText className="h-4 w-4 text-td-violet" /><span className="flex-1"><span className="block text-xs font-semibold text-td-primary">Private staff notes</span><span className="mt-1 block text-[11px] text-td-muted">Internal context—never shown to the customer.</span></span><ChevronDown className={`h-4 w-4 text-td-muted transition ${notesOpen ? "rotate-180" : ""}`} /></button>{notesOpen ? <div className="px-4 pb-4"><textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} rows={3} placeholder="Preferences, conversations, or service context…" className="workspace-input w-full resize-none" /></div> : null}</div>
        {duplicateWarning ? <div className="mt-4 rounded-xl border border-td-warning/20 bg-td-warning/[0.06] p-3 text-[11px] leading-5 text-td-warning"><strong className="block">Possible duplicate customer</strong>{duplicateWarning}<span className="mt-1 block text-td-warning/60">Submit again only if this is intentionally a separate profile.</span></div> : null}
      </div>
      <div className="flex shrink-0 items-center justify-end gap-2 border-t border-td-ink/[0.07] bg-td-surface/95 px-5 py-4 backdrop-blur-xl sm:px-6"><button type="button" onClick={() => setEditorOpen(false)} className="h-11 rounded-xl border border-td-ink/[0.09] px-5 text-xs font-semibold text-td-secondary transition hover:bg-td-ink/[0.04]">Cancel</button><button disabled={saving} className="inline-flex h-11 min-w-44 items-center justify-center gap-2 rounded-xl bg-gradient-to-b from-td-accent to-td-accent px-5 text-xs font-bold text-td-on-accent shadow-[0_12px_28px_rgb(var(--td-accent-rgb)/.18)] disabled:opacity-50">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />}{saving ? "Saving securely…" : editingId ? "Save changes" : "Create customer profile"}</button></div>
    </form></Modal> : null}

    {selected ? <div className="fixed inset-0 z-50 flex justify-end bg-black/70 backdrop-blur-sm" onMouseDown={(e) => { if (e.target === e.currentTarget) setSelected(null); }}><aside className="h-full w-full max-w-xl overflow-y-auto border-l border-td-accent/[0.1] bg-td-surface p-5 shadow-2xl sm:p-7"><ModalHeader title={`${selected.first_name} ${selected.last_name}`} subtitle={selected.email || selected.phone || "Customer profile"} onClose={() => setSelected(null)} /><div className="mt-5 flex gap-2"><button type="button" onClick={() => openEdit(selected)} className="flex h-10 flex-1 items-center justify-center gap-2 rounded-xl border border-td-ink/[0.08] bg-td-ink/[0.025] text-xs text-td-primary"><Pencil className="h-3.5 w-3.5" /> Edit profile</button><button type="button" aria-label={`Delete ${selected.first_name} ${selected.last_name}`} onClick={() => void deleteCustomer(selected)} className="flex h-10 w-10 items-center justify-center rounded-xl border border-td-danger/10 text-td-danger/70"><Trash2 className="h-4 w-4" /></button></div><div className="mt-5 grid grid-cols-2 gap-3"><ProfileStat label="Store credit" value={money(selected.store_credit_cents)} tone="emerald" /><ProfileStat label="Loyalty points" value={selected.loyalty_points.toLocaleString()} tone="amber" /><ProfileStat label="Lifetime spend" value={money(selected.lifetime_spend_cents)} /><ProfileStat label="Orders" value={String(selected.order_count)} /></div>
      <section className="mt-5 rounded-2xl border border-td-ink/[0.07] bg-td-ink/[0.018] p-4"><div className="flex items-center gap-2"><MessageSquareText className="h-4 w-4 text-td-accent-text" /><h3 className="text-sm font-semibold text-td-primary">Account adjustment</h3></div><form onSubmit={adjustBalance} className="mt-4 grid gap-3"><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => setAdjustment({ ...adjustment, type: "credit" })} className={`h-9 rounded-xl text-xs font-semibold ${adjustment.type === "credit" ? "bg-td-success/10 text-td-success ring-1 ring-td-success/20" : "bg-td-ink/[0.025] text-td-muted"}`}>Store credit</button><button type="button" onClick={() => setAdjustment({ ...adjustment, type: "points" })} className={`h-9 rounded-xl text-xs font-semibold ${adjustment.type === "points" ? "bg-td-warning/10 text-td-warning ring-1 ring-td-warning/20" : "bg-td-ink/[0.025] text-td-muted"}`}>Loyalty points</button></div><TextField required type="number" label={adjustment.type === "credit" ? "Amount in dollars (negative removes)" : "Points (negative removes)"} value={adjustment.amount} onChange={(value) => setAdjustment({ ...adjustment, amount: value })} /><TextField required label="Reason / reference" value={adjustment.reason} onChange={(value) => setAdjustment({ ...adjustment, reason: value })} /><button disabled={saving} className="h-10 rounded-xl bg-td-accent text-xs font-bold text-td-on-accent disabled:opacity-50">Post adjustment</button></form></section>
      <section className="mt-5"><h3 className="text-sm font-semibold text-td-primary">Account ledger</h3><div className="mt-3 space-y-2">{ledger.length ? ledger.map((entry) => <div key={entry.id} className="flex items-center gap-3 rounded-xl border border-td-ink/[0.06] bg-td-ink/[0.015] p-3"><span className={`flex h-9 w-9 items-center justify-center rounded-xl ${entry.entry_type === "credit" ? "bg-td-success/[0.07] text-td-success" : "bg-td-warning/[0.07] text-td-warning"}`}>{entry.entry_type === "credit" ? <BadgeDollarSign className="h-4 w-4" /> : <Star className="h-4 w-4" />}</span><span className="min-w-0 flex-1"><span className="block truncate text-xs font-medium text-td-primary">{entry.reason}</span><span className="mt-1 block text-[11px] text-td-muted">{new Date(entry.created_at).toLocaleString()}</span></span><span className={`text-xs font-bold ${entry.amount >= 0 ? "text-td-success" : "text-td-danger"}`}>{entry.amount >= 0 ? "+" : ""}{entry.entry_type === "credit" ? money(entry.amount) : `${entry.amount} pts`}</span></div>) : <p className="rounded-xl border border-dashed border-td-ink/[0.07] p-5 text-center text-xs text-td-muted">No adjustments yet.</p>}</div></section>
    </aside></div> : null}
  </WorkspaceFrame>;
}
...
function ModalHeader({ title, subtitle, onClose }: { title: string; subtitle: string; onClose: () => void }) { return <div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-semibold text-td-primary">{title}</h2><p className="mt-1 text-[0.8rem] text-td-muted">{subtitle}</p></div><button type="button" aria-label="Close panel" onClick={onClose} className="flex h-9 w-9 items-center justify-center rounded-xl border border-td-ink/[0.07] text-td-muted"><X className="h-4 w-4" /></button></div>; }
function TextField({ label, value, onChange, type = "text", required = false, placeholder }: { label: string; value: string; onChange: (value: string) => void; type?: string; required?: boolean; placeholder?: string }) { return <label><span className="text-[0.68rem] font-semibold uppercase tracking-[0.12em] text-td-secondary">{label}</span><input required={required} type={type} value={value} placeholder={placeholder} onChange={(e) => onChange(e.target.value)} className="workspace-input mt-2 w-full placeholder:text-td-muted" /></label>; }
function NumberField({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) { return <label><span className="text-[0.68rem] font-semibold uppercase tracking-[0.12em] text-td-muted">{label}</span><input min="0" type="number" value={value} onChange={(e) => onChange(Number(e.target.value))} className="workspace-input mt-2 w-full" /></label>; }
function SelectField({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: readonly (readonly [string,string])[] }) { return <label><span className="text-[0.68rem] font-semibold uppercase tracking-[0.12em] text-td-secondary">{label}</span><select value={value} onChange={(e) => onChange(e.target.value)} className="workspace-input mt-2 w-full appearance-none">{options.map(([optionValue, optionLabel]) => <option key={optionValue} value={optionValue}>{optionLabel}</option>)}</select></label>; }
function FormSection({ icon: Icon, title, description, children }: { icon: typeof UserRound; title: string; description: string; children: React.ReactNode }) { return <section className="mb-4 rounded-2xl border border-td-ink/[0.07] bg-td-ink/[0.018] p-4"><div className="mb-4 flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl border border-td-accent/[0.12] bg-td-accent/[0.05] text-td-accent-text"><Icon className="h-4 w-4" /></span><div><h3 className="text-sm font-semibold text-td-primary">{title}</h3><p className="mt-0.5 text-[0.8rem] text-td-muted">{description}</p></div></div>{children}</section>; }
function ConsentToggle({ label, detail, checked, onChange }: { label: string; detail?: string; checked: boolean; onChange: (checked: boolean) => void }) { return <button type="button" aria-pressed={checked} onClick={() => onChange(!checked)} className={`flex items-center gap-3 rounded-xl border p-3 text-left transition ${checked ? "border-td-success/20 bg-td-success/[0.07] text-td-success" : "border-td-ink/[0.07] bg-td-ink/[0.02] text-td-secondary hover:border-td-ink/[0.13]"}`}><span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-md ${checked ? "bg-td-success text-td-on-accent" : "border border-td-ink/15"}`}>{checked ? <Check className="h-3.5 w-3.5" /> : null}</span><span><span className="block text-[11px] font-semibold">{label}</span>{detail ? <span className="mt-0.5 block text-[11px] text-td-muted">{detail}</span> : null}</span></button>; }
...
function ProfileStat({ label, value, tone }: { label: string; value: string; tone?: "emerald" | "amber" }) { return <div className="rounded-2xl border border-td-ink/[0.06] bg-td-ink/[0.018] p-4"><p className="text-[0.68rem] font-bold uppercase tracking-[0.12em] text-td-muted">{label}</p><p className={`mt-2 text-lg font-semibold ${tone === "emerald" ? "text-td-success" : tone === "amber" ? "text-td-warning" : "text-td-primary"}`}>{value}</p></div>; }
function AudienceStat({ label, value }: { label: string; value: number }) { return <div className="rounded-xl border border-td-ink/[0.06] bg-td-ink/[0.018] p-3"><p className="text-[0.68rem] uppercase tracking-[0.12em] text-td-muted">{label}</p><p className="mt-1 text-lg font-semibold text-td-primary">{value}</p></div>; }
